...
impl Applier<Cad, MetaAnalysis> for ListApplier {
    fn apply_one(
        ...
        let ids: Vec<Id> = get_meta_list!(egraph, map[self.var]).as_vec().clone();
...
impl Applier<Cad, MetaAnalysis> for SortApplier {
    fn apply_one(
        ...
        let items = get_meta_list!(egraph, map[self.list]).as_vec();
...
impl Applier<Cad, MetaAnalysis> for PartApplier {
    fn apply_one(
        ...
        let list_of_lists = part
            ...
            .collect();
        let list_of_lists = VecId::new(list_of_lists);
