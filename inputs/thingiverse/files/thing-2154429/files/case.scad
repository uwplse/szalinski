...
module top() {
    ...
    module plugs(extra) { 
       ...
       usb_plug(usb2dy);
        ...
        // unicorn hat opening
        if (have_unicorn) translate([10, 0, pcb2roof - extra]) 
			c_cube(unicorn_box, unicorn_box, wall+2*extra);
                ...
                if (have_unicorn) { 
                    translate([0, 0, 10]) {
                        for (dx = [pcbmnt1dx,pcbmnt2dx]) for (dy = [pcbmnt1dy,pcbmnt2dy]) translate([dx, dy, 0]) {
                            cylinder(r1 = pcbmntdia/2 +wall, r2 = pcbmntdia/2 +wall, h = pcb2roof-10, $fs=0.2);
                        }
                    }
                } else {
                     for (dx = [pcbmnt1dx,pcbmnt2dx]) for (dy = [pcbmnt1dy,pcbmnt2dy]) translate([dx, dy, 0]) {
