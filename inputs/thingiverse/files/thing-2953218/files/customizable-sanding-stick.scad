...
end_1_wedge_angle = 15.5; // [5:75]

// If you want a sharp end of the wedge, set 0. If you want a round end, set > 0. 
end_1_wedge_round_tip_diameter_in_millimeter = 0.5;
...
screw_hole_dia_in_millimeter = 4.3;
...
module base_bottom(length, height, width) {
    difference() {
     ...
     // Hole 1 
     translate([4 + screw_head_space / 2, - width / 2, 1.5+2.9]) hexaprism(3.5,3.5);   
...
*/        
    // Hole 2
    translate([length - 4 - screw_head_space / 2, - width / 2, 1.5+2.9]) hexaprism(3.5,3.5);
/*        if(height < max_thread_height + 1.5) {
