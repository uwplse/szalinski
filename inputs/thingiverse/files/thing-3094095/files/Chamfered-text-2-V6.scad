//Set the 1st textline to render
textstring1="A";

//Set the 2nd textline to render - or leave blank if only 1 line is needed
textstring2="B";
...
//line height in font size measured
lineheight=1.0;
...
module RiseText(textstr1, textstr2, textsize,textheight, scale) 
{ 
    minkowski()
    {
        union()
        {
            if (textstr1!="")
                translate([0,textsize*lineheight,0])
                    linear_extrude(height=.0001,convexity = 32)
                        writetext(textstr1, textsize);
            
            if (textstr2!="")
                linear_extrude(height=.0001,convexity = 32)
                    writetext(textstr2, textsize);
        }
        
        //rotate([0,180,0])
            cylinder(h=textheight, r1=scale, r2=0, center=true, $fn=8);
    }
}

//rotate([90,0,0])
    RiseText(textstring1,textstring2,textsize,textheight, scale);
