...
Part = "a"; // [a:Buffers, b:MotorMount, c:Gear, d:JawRound, e:JawInsert, f:JawSquare]

//Adjust this to snug fit for the star grip
slip_star     = 0.25;       

//The greatest diameter supported by the buffers
diameter_buffer = 25;       

//Diameter of round stock. Adjust to snug fit.
diameter_round_stock = 25;  

//Inner diameter of bearing. Adjust to snug fit.
diameter_bearing_id = 8;    

//Diameter of cylinder going into your stock
diameter_insert_stock = 10; 

//Width of round stock. Adjust to snug fit.
width_square_stock = 20;    

//Depth of round stock. Adjust to snug fit.
depth_square_stock = 20;    



if(Part=="a"){
    backend(diameter_buffer,false);
    translate([45,0,0]){backend(diameter_buffer,true);}
}else if (Part=="b"){
    motormount();
}else if (Part=="c"){
    passive_end(diameter_bearing_id);
    translate([22,0,3]){active_end(diameter_bearing_id);}
}else if (Part=="d"){
    translate([0,0,3]){ rotate([180,0,0]){ stock_round(diameter_round_stock);}}
    translate([diameter_round_stock+10,0,3]){ rotate([180,0,0]){ stock_round(diameter_round_stock);}}
}else if (Part=="e"){
    translate([0,0,3]){ rotate([180,0,0]){                       stock_inner(diameter_insert_stock);}}
    translate([diameter_round_stock+10,0,3]){ rotate([180,0,0]){ stock_inner(diameter_insert_stock);}}
}else if (Part=="f"){
    translate([0,0,3]){ rotate([180,0,0]){                       stock_square(width_square_stock,depth_square_stock);}}
    translate([max(width_square_stock,depth_square_stock)+10,0,3]){ rotate([180,0,0]){ stock_square(width_square_stock,depth_square_stock);}}
}
...
module motormount(){
    ...
    difference(){
        ...
        union(){
            translate([22-4,0,side/2+2]){
                cube([44,side+8,side+4],center=true);
            }
            translate([22-4,0,2]){cube([60,side+8,4],center=true);}
        }

        translate([0,0,side/2+4+0.01]){
            rotate([0,90,0]){
                translate([-side/2,-side/2,0]){
                    cube([side,side,40]);    
                }
                translate([hole,hole,-5]){cylinder(d=3.4,h=5);}
                translate([-hole,-hole,-5]){cylinder(d=3.4,h=5);}
                translate([hole,-hole,-5]){cylinder(d=3.4,h=5);}
                translate([-hole,hole,-5]){cylinder(d=3.4,h=5);}    
                translate([0,0,-5]){cylinder(d=26,h=5);}
            }
        }
       
       translate([41.7,0,54]){
           rotate([0,45,0]){cube([70,70,70],center=true);}
       }
       
       //mount holes
       translate([-10,1,-1]){cube([4,22,6]);}
       translate([-10,-23,-1]){cube([4,22,6]);}       
       translate([42,1,-1]){cube([4,22,6]);}
       translate([42,-23,-1]){cube([4,22,6]);}
...
module backend(backdia,fixed){

he = backdia/2+2+2;

    difference(){
    union(){    
        translate([-10,-16,0]){cube([10,32,he+15]);}
        translate([0,2,0]){  tri(he);}
        translate([0,16,0]){ tri(he);}
        translate([0,-16+4,0]){tri(he);}
        translate([-30,-24,0]){cube([40,48,5]);}
    }
    translate([0.01,0,he]){rotate([0,90,0]){cylinder(d=backdia+6,h=50);}}
    translate([0.01,0,he]){rotate([0,-90,0]){cylinder(d=22.3,h=7);}}

    if(fixed){  
        translate([5,-20,-0.1]){    cylinder(d=4,h=10); }
        translate([5,+20,-0.1]){    cylinder(d=4,h=10); }    
        translate([-25,-20,-0.1]){    cylinder(d=4,h=10); }
        translate([-25,+20,-0.1]){    cylinder(d=4,h=10); }
    }else{
        translate([-8,-22,-0.1]){    cube([16,4,10]); }
        translate([-8,+18,-0.1]){    cube([16,4,10]); }    
        translate([-28,-22,-0.1]){    cube([16,4,10]); }
        translate([-28,+18,-0.1]){    cube([16,4,10]); }
    }
                    
    translate([0,0,he+15]){cube([30,2,1],center=true);}       
}

    //support
   /* difference(){
        translate([0.4,0,he]){rotate([0,-90,0]){cylinder(d=22.3,h=0.8);}}
        cube([4,30,40],center=true);   
    }*/
}
...
module tri(he){
    rotate([90,0,0]){    
        linear_extrude(4){    
            polygon(points=[[-30,5],[-10,5],[-10,he+15]]);
        }
    }
}
...
module stock_round(dia){
   difference(){
        translate([0,0,-2]){ cylinder(d=dia+4,h=5); }    
        translate([0,0,-5]){cylinder(h=5,d=dia);}
        minkowski(){
            translate([0,0,1.1]){star(2);}
            cylinder(d=slip_star,h=0.1);
        }
    }
}

module stock_inner(dia){
   difference(){
        translate([0,0,-2]){ cylinder(d=25,h=5); }    
        translate([0,0,-5]){cylinder(h=5,d=35);}
        minkowski(){
            translate([0,0,1.1]){star(2);}
            cylinder(d=slip_star,h=0.1);
        }
    }
    translate([0,0,-2]){cylinder(d=dia,h=2);}
}

module stock_square(w,d){
   difference(){
        translate([0,0,0]){ cube([w+4,d+4,5],center=true); }//cylinder(d=25,h=5); }    
        translate([0,0,-2.5]){ cube([w,d,5],center=true); } //cylinder(h=5,d=35);}
        minkowski(){
            translate([0,0,1.1]){star(2);}
            cylinder(d=slip_star,h=0.1);
        }
...
module active_end(id){
    intersection(){
        union(){
            pulley();         
            translate([0,0,-1.5-1.5]){ star(1.5); }
        }
        translate([0,0,-5]){cylinder(d=20,h=30);}
    }
        translate([0,0,8]){cylinder(d=id,h=5);}
...
module passive_end(id){
    star(1.5);    
    translate([0,0,1.5]){cylinder(d=20,h=1.5);}
    translate([0,0,1.5]){cylinder(d=id,h=6);}
}
    ...
    intersection(){    
        for(i=[0:45:360]){
            rotate([0,0,i]){
                ...
                polygon(points=[[0,0],[-2,11],[2,11]]); }
            }
        } 
        translate([0,0,-1]){cylinder(d=20,h=he+2);}
    } 
    translate([0,0,-1]){cylinder(d=8,h=he+2);}
    }   
    cylinder(d=4,h=he);
}
