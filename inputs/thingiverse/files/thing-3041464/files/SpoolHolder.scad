//custom spool holder
//designed for wanhao i3 plus mark 2 printer
//for others: play with it - not any values would work correct...

//axis diameter
SpDia=20;
//konus diameter
SpKoDia=38;
//axis width (konus will be added)
SpW=80;
//diameter of the bolt
BoDia=30;
//bolt lenght
BoL=20;
//make the nut thinner (normal = 1)
NutThin=0.6; 

//fineness of round things
$fn=60;
// =0= Rolson sizes, =1= Fairbury sizes (ISO Metric Thread Implementation)
WrenchSizes=0;

//spoolholder
translate([0,BoDia,0]){
	translate([0,0,SpDia/2])
	cylinder(h=SpW,d=SpDia);
	cylinder(h=SpDia/2,d1=SpKoDia,d2=SpDia);
	translate([0,0,SpW+SpDia/2])
	cylinder(h=SpDia/2,d2=SpKoDia,d1=SpDia);
}

//thread
translate([0,BoDia,SpW+BoL+SpDia-1]){
	rotate([0,180,0]){
		thread_out(BoDia,BoL);
		thread_out_centre(BoDia,BoL);
	}
}
...
// For thread dimensions see http://en.wikipedia.org/wiki/File:ISO_and_UTS_Thread_Dimensions.svg
...
//  dia=diameter, 6=M6 etc.
{
	hi = rolson_hex_nut_hi(dia) * NutThin;
	difference()
	{
...
//  dia=diameter, 6=M6 etc.
{
	hi = fairbury_hex_nut_hi(dia) * NutThin;
	difference()
	{
