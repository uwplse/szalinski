// text used text_on libery : https://github.com/brodykenrick/text_on_OpenSCAD

//Thikness of the wall
Wall = 5;
InnerWall = (Wall-2*.4);
//Heigt of the Cylinder
Height = 150;
//Size in diameter
Size = 50;
Hole_Size = 15;

//See if placing of font etc is ok
Font_Preview = "yes"; //[yes,no]

Text = "TkkrLab";

//If you want a different font check https://fonts.google.com/
Font = "OCR A Extended:style=Regular";
Font_Size = 20;
...
if (Font_Preview == "yes") { 
    text_on_cylinder(Text,font=Font,[0,0,Height/2],r=Size-Wall,h=1, extrusion_height=Wall*3, size=Font_Size);
}
...
union() {
    difference() {
        //main object
        difference() {
          cylinder(Height,Size,Size); //main outer cylinder
          translate([0,0,2]) cylinder(Height,r=Size-Wall);  //inner cylinder
          cylinder(2*Wall,r=Hole_Size,true); //bottom hole
        }
        //make main cylinder hollow
        translate([0,0,Wall]) difference() {
            cylinder(Height-2*InnerWall,r= Size - Wall + 4 );  //inner cylinder
            cylinder(Height-2*InnerWall,r= Size - 4.5); 
        }
    }

    //make text inside cylinder
    difference() {
        text_on_cylinder(Text,font=Font,[0,0,Height/2],r=Size-Wall,h=1, extrusion_height=Wall, size=Font_Size);
        //inner cylinder
        translate([0,0,2]) cylinder(Height,r=Size-Wall);  
        //remove outer part 
        difference() {
            cylinder(Height,r=2*Size); //main outer cylinder
            cylinder(Height,r=Size); //main outer cylinder
        }    
    }
};
