...
// remix by faggahz 2019: add feet and mid element
//
// TODO:
...
part = "all"; // [ top, bottom, button, buttons, all, mid ]
with_battery = 0; // [ 1:true, 0:false ] 
// size of extra room, if any, set to 0 otherwise
extra_room = 0;  
have_feet = 1; // [ 1:true, 0:false ] 
...
module bottom() {
	...
	module add() {
        ...
        if (have_feet){ for (x = [-1, 1]) 
            hull(){
            for (y = [-1, 1])
           			translate([x*(pcb[0]/2+tol+wall-corner_r-10) + (x>0 ? extra_y : 0), y*(pcb[1]/2+tol+wall-corner_r+10), 0])cylinder(3,5,5); 
        }
        }
	}
	module sub() {
		...
		difference () {
            ...
            translate([pcbmnt2dx, pcbmnt2dy+4, pcb2floor-pcb2lower-.9])cube([2,pcb2floor-pcb2lower,pcb2floor-pcb2lower],center=true);
            
            pedestal(pcbmnt2adx, -pcbmnt2dy, pcb2floor-pcb2lower, pcbmnt2dia);
            translate([pcbmnt2adx, -pcbmnt2dy-4, pcb2floor-pcb2lower-.9])cube([2,pcb2floor-pcb2lower,pcb2floor-pcb2lower],center=true); 
            
            pedestal(pcbmnt3dx, pcbmnt3dy, pcb2floor-pcb2lower, pcbmnt2dia);
            translate([pcbmnt3dx+2, pcbmnt3dy, pcb2floor-pcb2lower-.9])cube(pcb2floor-pcb2lower,center=true);  
            
            pedestal(pcbmnt3dx, pcbmnt3ady, pcb2floor-pcb2lower, pcbmnt2dia);
            translate([pcbmnt3dx+2, pcbmnt3ady, pcb2floor-pcb2lower-.9])cube(pcb2floor-pcb2lower,center=true);     
        ...
        if (have_feet){ for (x = [-1, 1]) 
           for (y = [-1, 1])
           translate([x*(pcb[0]/2+tol+wall-corner_r-10) + (x>0 ? extra_y : 0), y*(pcb[1]/2+tol+wall-corner_r+10), 0])cylinder(10,2,2,center=true); 
...
module top() {
	...
	for (dx = [-pcbmntdx, pcbmntdx]) for (dy = [-pcbmntdy, pcbmntdy])
		translate([lcdpcbdx + dx, lcdpcbdy + dy, pcb2roof-lcdpcb2roof]) {
         ...
         translate([lcdpcbdx + pcbmntdx+2, lcdpcbdy + pcbmntdy, pcb2roof-lcdpcb2roof]) c_cube(pcb[0]/2-pcbmnt3dx, wall, lcdpcb2roof); // support
...
module mid() {
	...
	module add() {
		hull () for (x = [-1, 1]) for (y = [-1, 1]) {
			translate([x*(pcb[0]/2+tol+wall-corner_r) + (x>0 ? extra_y : 0), y*(pcb[1]/2+tol+wall-corner_r), -frame_w]) 
				cylinder(r = corner_r+tol+wall, h = d, $fs=0.3); // include frame
			translate([x*(pcb[0]/2+tol+wall-corner2_r) + (x>0 ? extra_y : 0), y*(pcb[1]/2+tol+wall-corner2_r), hmid-05]) 
					sphere(r = corner2_r, $fs=0.3);	
		}
	}
	module add2() {
		hull () for (x = [-1, 1]) for (y = [-1, 1])
			translate([x*(pcb[0]/2+tol+wall-corner_r) + (x>0 ? extra_y : 0), y*(pcb[1]/2+tol+wall-corner_r), 0+9.7]) {
//				sphere(r = corner_r, $fs=0.3);
				cylinder(r = corner_r, h = 10, $fs=0.3);
		}
            // snap lock
            if(with_battery){
            if(extra_room){
            snap2(batt[0]+2*wall+2*tol+extra_room);
            }else{
            snap2(batt[0]+1*wall+2*tol);    
            }}else{
            if(extra_room){
            snap2(1*wall+1*tol+extra_room);         
            }else{
            snap2(0*wall+0*tol);         
            }}
	//	snap2(with_battery ? batt[0]+wall+2*tol : 0);
...
//		translate([0,0,13.5-hmid])snap2(with_battery ? batt[0]+wall+2*tol : 0);
		rotate([0, 0, 180]) translate([0,0,13.5-hmid])snap2(0);
		difference() { 
			// room for pcb
			translate([0, 0, -d]) cr_cube(2*tol+pcb[0], 2*tol+pcb[1], d+pcb2roof, 1.0);
		}
       ...
       if (with_battery) translate([pcb[0]/2+2*tol+wall+batt[0]/2+(extra_room>0 ? extra_room+wall : 0), 0, -d]) {
		...
		difference () {
			// pcb itself
			translate([-(pcb[0]/2+tol), -(pcb[1]/2+tol), wall-hmid])
				cube([2*tol+pcb[0], 2*tol+pcb[1], pcb2floor+pcb[2]+d+hmid]);
		}
         ...
         *   for (dx = [-5.0, 5.0]) translate([dx, pcb[1]/2 - 6.0/2, pcb2floor*.5]) rotate([0, -90, -90]) cylinder(r=6.0/2, h=extra_room, $fn=24); // holes in external wall
            difference () {
                translate([0,0,-hmid])
                c_cube(extra_room, pcb[1]+2*tol, batt[2]+2*tol+hmid);
                for (dy = [0, 20]) translate([0, dy, 0]) rotate([45, 0, 0]) {
                    for (dx = [(extra_room/2-wall/2), -(extra_room/2-wall/2)]) { 
                        translate([dx, 0, 0]) c_cube(wall, wall, 30);
                        translate([dx, pcb[2] + 2*tol + wall, 0]) c_cube(wall, wall, 30);
        ...
        if (with_battery) translate([pcb[0]/2+2*tol+wall+batt[0]/2+(extra_room>0 ? extra_room+wall : 0), 0, wall]) {
             translate([0,0,-hmid])
            c_cube(batt[0]+2*tol, pcb[1]+2*tol, batt[2]+2*tol+hmid);
            translate([0, pcb[1]/2 - 4.0/2, pcb2floor*.8]) rotate([0, -90, 0]) cylinder(r=4.0/2, h=batt[0]); // hole in internal wall
        }
	}
	...
	difference () {
        union()
        {
        translate([0,0,+12-hmid])add();
		translate([0,0,-7])add2();
        }
        translate([0,0,+12-hmid])sub();
	    translate([0,0,-7])sub2();
...
if (part=="mid" || part=="all") translate([-0, 105, 13])rotate([180,0,0]) mid();	