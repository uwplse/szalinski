...
// Would you like the currency symbol to be written (instead of the image. Image taken by default)
symbol = 1; //// [1:Yes please, 0:No thanks]

// Which one would you like to see?
part = "tubes"; // [all:All these three parts assembled together,all_unassembled:All these three parts unassembled,basebox:Base box only,topboard:Top board only,tubes:Tubes only]
...
enable_box = (part == "all" || part == "all_unassembled" || part == "basebox");
  ...
  enable_symbol = (symbol == 1) && !enable_image;
...
module base_box() {
  ...
  difference() {
    ...
    if (enable_symbol) {
      //symbol_left_subtraction();
      symbol_right_subtraction();
    }
    if (enable_mesh) {
...
/**
 * Why $ is mirrored printed?!
 * Why ternaire operator works when a basic if/else doesn't... ??!!
 */
module symbol_right() {
  coin_symbol = currency == "usd" ? "$" :
        currency == "eur" ? "€" :
        currency == "chf" ? "CHF" :
        currency == "cad" ? "C$" :
        currency == "thb" ? "B." /*฿*/ : "?";
...
//echo("currency=", currency);
  ...
  rotate_y    = (currency == "usd") ? 180 : 0;
  translate_x = (currency == "usd") ? image_right_size : 0;
  
  /*if (currency == "usd") {
      rotate_y    = 180;
      translate_x = image_right_size;
  } else {
      rotate_y    = 0;
      translate_x = 0;
  }*/
    
//echo("rotate_y=", rotate_y);
//echo("translate_x=", translate_x);
  ...
  translate([board_length - 1,
    ...
    resize([image_right_size, image_right_size, 1]) {
      linear_extrude(height = 0.5) { 
          rotate([0, rotate_y, 0]) text(coin_symbol); 
      }
    }
...
module symbol_right_subtraction() {
  image_right_subtraction();
}
