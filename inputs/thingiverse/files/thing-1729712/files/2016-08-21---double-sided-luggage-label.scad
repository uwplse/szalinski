/*
OpenSCAD-File to generate a Customized luggage label with two sides
2013-11-01 - kowomike
2016-08-21 - jspiro
...
back_line_1="And also";
back_line_2="something";
back_line_3="on the back";
back_line_4="";
back_line_5="";
back_line_6="";
back_line_7="";
...
text_style=10; // [-80:Deep engraving (80 %),-50:Half way through engraving (50 %),-10:Shallow engraving (10 %),0: Hide text inside (takes 66 %),10:Little embossing (10 %),25:Heavy embossing (25 %) ]

//Choose back text engraving
back_text_style=-80; // [-80:Deep engraving (80 %),-50:Half way through engraving (50 %),-10:Shallow engraving (10 %)]
...
maxtextchars = max(len(text[0]),len(text[1]),len(text[2]),len(text[3]),len(text[4]),len(text[5]),len(text[6]),len(text[7]),len(text[8]),len(text[9]),len(back_text[0]),len(back_text[1]),len(back_text[2]),len(back_text[3]),len(back_text[4]),len(back_text[5]),len(back_text[6]),len(back_text[7]),len(back_text[8]),len(back_text[9]));
...
len(text[1])>0 ? 2 : 0,
len(text[0])>0 ? 1 : 0,
len(back_text[9])>0 ? 10 : 0,
len(back_text[8])>0 ? 9 : 0,
len(back_text[7])>0 ? 8 : 0,
len(back_text[6])>0 ? 7 : 0,
len(back_text[5])>0 ? 6 : 0,
len(back_text[4])>0 ? 5 : 0,
len(back_text[3])>0 ? 4 : 0,
len(back_text[2])>0 ? 3 : 0,
len(back_text[1])>0 ? 2 : 0,
len(back_text[0])>0 ? 1 : 0,
];

textlines=max(tl[0],tl[1],tl[2],tl[3],tl[4],tl[5],tl[6],tl[7],tl[8],tl[9],tl[10],tl[11],tl[12],tl[13],tl[14],tl[15],tl[16],tl[17],tl[18],tl[19]);
...
difference () {
  ...
  //Backside engraving
  translate ([margin+insertlength,-lineheight*(textlines-2)-(linespacing-1)*lineheight*(textlines-1)-margin+corner_radius,0])
	mirror([0,0,0]) rotate([180,0])
    union () {
    for (i=[0:textlines-1]) {
    translate ([0,-lineheight*i+descenderfactor*lineheight-(linespacing-1)*lineheight*i,plate_thickness/2+plate_thickness*back_text_style/100-0.05])
    write(back_text[i],t=abs(back_text_style)*plate_thickness/100+0.1,h=fontsize,font=font,space=letterspacing);
    }
  }
