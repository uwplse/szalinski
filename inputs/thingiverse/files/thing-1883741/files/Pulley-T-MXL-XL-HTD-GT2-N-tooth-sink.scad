...
m3_sink = 1; // Countersink mounting screw, 0 = No, 1 = Yes
m3_sink_dia = 5.6; // normal M3 socket cap = max 5.5mm
m3_sink_depth = 3; // normal M3 socket cap height = 3mm
...
pulley_t_ht = 7.4;	// length of toothed part of pulley, standard = 12
	 ...
	 {	 
		...
		if ( (pulley_b_dia - motor_shaft)/2 < m3_nut_depth + 3 ) { echo ("CAN'T DRAW CAPTIVE NUTS, DIAMETER TOO SMALL FOR NUT DEPTH!!!"); } else {
			...
			union()
			{
                ...
                //sink hole
                if(m3_sink==1) {
                    if((pulley_b_dia-motor_shaft)/2 < (nut_shaft_distance+m3_nut_depth+m3_sink_depth+1)) {
                        echo("CAN'T DRAW COUNTERSINK, DIAMETER TOO SMALL FOR SINK DEPTH!!!");
                    } else {
                        translate([0, 0, pulley_b_dia/2-m3_sink_depth]) rotate([0,0,22.5]) cylinder(r=m3_sink_dia/2,h=m3_sink_depth,$fn=12);
                    }
                }
            }
		}}
