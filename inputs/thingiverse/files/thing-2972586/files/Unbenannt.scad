...
module cutter(angle,width){
    translate([0,0,py3/2])rotate([0,angle,0])cube([width,500,500],center = true);
}

module middle(){
    translate([-250,-250,0])cube([500,500,py3]);
}

//cutter(45,5);


for (i = [1 : abs(1) : number_of_lines]){
    rotate([0,0,i*360/number_of_lines])intersection(){
    bezier_vase(
    bottom_radius, 
    curve_width, 
    thickness, 
