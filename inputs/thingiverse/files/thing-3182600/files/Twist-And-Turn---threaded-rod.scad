// Kiddicraft Twist And Turn threaded rod generator
//
// https://www.thingiverse.com/thing:3182600
//
// by Erick Neilson - 247generator @ thingiverse - 2018
//
// based on the 'Nut Job', hacked for double ended non-threaded curved rod ends
// and customizer options optimized for the relevant settings only.
// 
// 
//
...
/* [Threaded Rod Options] */

//Length of the threaded section, 148.77mm for original full length rod.
thread_length  					= 148.77;	
//Outer diameter of the thread to mesh loosely with 23.5mm nuts
thread_outer_diameter           = 22.0;		
//Resolution (lower values for higher resolution slows rendering - 1.0 for the STLs)
resolution    					= 2.0;	

/* [Extended Options] */

//Length of the non-threaded section, 7.77mm default.
non_thread_length				= 7.77;	
//Thread step or Pitch (4.24mm matches the original product)
thread_step    					= 4.24;
//Step shape degrees (52 degrees is what generates the correct inner thread diameter)
step_shape_degrees 				= 52;	
...
{
    ntr=od/2-(st/2)*cos(lf0)/sin(lf0);
echo ("thread inner diameter", ntr*2);
    union()
    {
        hex_head(hg,df);
        ...
        else
        {
            if ( ntd == -1 )
            {
//!!!!!!!! HACK: rounded cylinder on one end of tbe cylinder
                rotate ([180,0,0]) translate ([0,0,-ntl-0.1])
                rcylinder(h=ntl+0.1, r=ntr, $fn=floor(od*PI/rs), b=2, center=false);
//!!!!!!!! HACK: double ended rounded blanks (top added)
                translate ([0,0,lt+ntl-0.1])
                rcylinder(h=ntl+0.1, r=ntr, $fn=floor(od*PI/rs), b=2, center=false);
            }
            else if ( ntd == 0 )
            {
...
// !!!!!!!!!!!!!!!!!!!! HACK: add
module rcylinder(r=10,h=10,b=2)
{
    r1=r;
    {translate([0,0,0]) 
    hull(){
        rotate_extrude() 
        translate([r-b,h-b,0]) intersection () { circle(r = b); square(b); }
        rotate_extrude() 
        translate([r-b/2, 0, 0]) square([b]);}
        }
    }
