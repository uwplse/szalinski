// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------
// Title:        Parametric Caged Bearing (Experimental Version)
// Version:      X-3.7
...
//
// - Version X-3.6
//   * Fixed throw angle bug.
//     - SCG operations to cut the hinge into a circular and semi-circular shape, were being performed after the throw
//       angle rotation was being applied to the leaves. This cased the model to be miss-constructed when throw angles were applied
//       by users.
//     - Problem solved by applying the SCG operations 'before' the throw angle operation is applied to the leaves in the assembly module.
//
// - Version X-3.5
//   * Added support for differential leaf gauges.
//
// - Version X-3.4
//   * Added support for circular hinges.
//     - Circular Hinge:
//       The entire hinge assembly is cut into a circle.
//     - Semi-Circular Leaves:
//       The knuckle joint cylinder is left unshaped, but the leaves are cut into semi-circles.
//   * New circular fastener layout also included, to accommodate circular hinge configurations.
//
// - Version 3.4
...
//     Useful for viewing the top and bottom pin shaft counterbores.
...
// Experimental feature constants.

C_NORMAL_SHAPE         = C_CONSTANT + 0;
C_CIRCULAR_HINGE       = C_CONSTANT + 1;
C_SEMI_CIRCULAR_LEAVES = C_CONSTANT + 2;
...
top_pin_shaft_counterbore_diameter    = 7.0;
top_pin_shaft_counterbore_depth       = 2.5;
top_pin_shaft_counterbore_shape       = 0;            // [ 0:Circular, 1:Square, 2:Hexagonal ]
bottom_pin_shaft_counterbore_diameter = 7.0;
bottom_pin_shaft_counterbore_depth    = 2.5;
bottom_pin_shaft_counterbore_shape    = 0;            // [ 0:Circular, 1:Square, 2:Hexagonal ]
...
hinge_shape                    = 2;     // [ 0:Normal, 1:Circular Hinge, 2:Semicircular Leaves ]
// Note: Knuckle gussets are not yet supported for differential gauge offset.
leaf_gauge_differential_offset = 0.0;     
...
m_hinge_shape                    = hinge_shape;
m_leaf_gauge_differential_offset = clip ( leaf_gauge_differential_offset, -m_leaf_gauge + 0.6, m_leaf_gauge - 1.6 );
...
{   
    ...
    if ( m_hinge_shape == C_NORMAL_SHAPE )
    {
        assembly_normal ();
    }
    else if ( m_hinge_shape == C_CIRCULAR_HINGE )
    {
        assembly_circular ();
    }
    else if  ( m_hinge_shape == C_SEMI_CIRCULAR_LEAVES )
    {
        assembly_circular ();
    }
}
...
// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------
// Module:      assembly_normal.
// Module Type: Assembly.
//
// Description:
//
// - Main component assembly module. All the components are assembled into the final product here.
//
// - Ideally, this module should only call modules defined as components.
//   Calling "Work Piece" or "Tool" modules here should be avoided.
//
// Parameters:
//
// - N/A
...
module assembly_circular ()
{
    ...
    if ( m_hinge_shape == C_CIRCULAR_HINGE )
    {   
        rotate ( [ 0.0, 0.0, ( m_flip_model ) ? 180.0 : 0.0 ] )
        {
            if ( m_female_leaf_enabled ) rotate ( [ 0.0, -m_throw_angle, 0.0 ] )
            {
                difference ()
                {
                    leaf ( C_FEMALE );
                    tool_cutter_circular_hinge ();
                }
            }
            
            if ( m_male_leaf_enabled )
            {
                difference ()
                {
                    leaf ( C_MALE );
    ...
    else if ( m_hinge_shape == C_SEMI_CIRCULAR_LEAVES )
    {
        rotate ( [ 0.0, 0.0, ( m_flip_model ) ? 180.0 : 0.0 ] )
        {
            if ( m_female_leaf_enabled ) rotate ( [ 0.0, -m_throw_angle, 0.0 ] )
            {
                ...
                {
                    leaf ( C_FEMALE );
                    tool_cutter_semi_circular_leaves ();
                }
            }
...
{   
    ...
    w_circular_hinge       = m_leaf_height/2.0;
    w_semi_circular_leaves = m_leaf_height/2.0 + m_leaf_gauge;
    
    w = ( m_hinge_shape == C_CIRCULAR_HINGE ) ? w_circular_hinge : ( m_hinge_shape == C_SEMI_CIRCULAR_LEAVES ) ? w_semi_circular_leaves : m_leaf_width;
    l = m_leaf_height;
    h = m_leaf_gauge;
...
module workpiece_leaf ( w, l, h, r, gender )
{
    c = ( gender == C_FEMALE ) ? m_leaf_gauge_differential_offset : -m_leaf_gauge_differential_offset;
...
{   
    ...
    union ()
    {
        ...
        union ()
        {
            ...
            translate ( [ 0, 0, c ] )
            {   
                cylinder ( d = od, h = m_leaf_gauge + 2*c );
            }
        }
    }
}
...
module tool_cutter_fastener_set ( fastener_count, fastener_column_count, z_offset )
{
    if ( m_hinge_shape == C_NORMAL_SHAPE )
    {
        tool_cutter_fastener_set_normal ( fastener_count, fastener_column_count, z_offset );
    }
    else
    {
        tool_cutter_fastener_set_circular ( fastener_count, fastener_column_count, z_offset );        
    }
}

// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------
// Module:      tool_cutter_fastener_set_normal
// Module Type: Cutting Tool.
// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------

module tool_cutter_fastener_set_normal ( fastener_count, fastener_column_count, z_offset )
{
    // Relative origin.
    ...
    // Two columns of fastener holes, if we have selected two fastener hole columns.
...
// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------
// Module:      tool_cutter_fastener_set_circular
// Module Type: Cutting Tool.
//
// TODO:
// - Fix clipping error. 
//   When the number of fasteners is clipped down to a number of fasteners that can fit on the leaf, in the
//   constraints section at the beginning of the file, the angular step size is being computed incorrectly.
//   - Looks like a fence post error.
//   - Only occurs when using a single column for fasteners.
//
// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------

module tool_cutter_fastener_set_circular ( fastener_count, fastener_column_count, z_offset )
{    
    // Compute origin.
    
    ox = ( m_hinge_shape == C_CIRCULAR_HINGE ) ?  0.0 : m_leaf_gauge;
    oy = m_leaf_height/2.0;
            
    // Compute geometric parameters.
    
    r0_circular      = ( m_leaf_height - m_fastener_head_diameter )/2.0 - m_fastener_margin;
    r0_semi_circular = r0_circular;
    r0               = ( m_hinge_shape == C_CIRCULAR_HINGE ) ? r0_circular : r0_semi_circular;
    r1               = m_fastener_head_diameter/2.0;
    m                = ( m_hinge_shape == C_CIRCULAR_HINGE ) ? m_leaf_gauge + m_fastener_margin : m_fastener_margin; 
    
    // Compute location of the first fastener hole.
    
    t0 = asin ( ( r1 + m )/r0 );    // Solve, r1 + m = r0 * sin ( t ), for t.
    x0 = m_leaf_gauge + m_fastener_margin + m_fastener_head_diameter/2.0;
    y0 = r0*sin ( t0 - 90 );
    
    
    // Compute fastener count distribution between the straight (diameter) column, and the curved (circumference) column.
    
    pi_n    = 22.0;                                                // Pi numerator (Circle circumference).
    pi_d    = 7.0;                                                 // Pi denominator ( Circle diameter).
    pi_s    = pi_n/2.0 + pi_d;                                     // Sum of, half a circle circumference and its diameter.
    n1_col1 = fastener_count - 1;                                  // Fastener count for single column fastener arrangement. 
    n2_col1 = round ( ( pi_d/pi_s )*( fastener_count ) );          // Fastener count for 1st column of two column arrangement.
    n2_col2 = round ( ( pi_n/( 2.0*pi_s ) )*( fastener_count ) );  // Fastener count for 2nd column of two column arrangement.
    ...
    if ( fastener_column_count == 1 )
    {
        ...
        for ( t = [ 0 : n ] )
        {   
            xd = ( m_hinge_shape == C_CIRCULAR_HINGE ) ? 0.0 : m_leaf_gauge; 
            yd = -r0;
            rz = t*( 180.0 - 2.0*t0 )/n + t0;
                        
            translate ( [ xd, 0.0, 0.0 ] ) rotate ( [ 0.0, 0.0, rz ] ) translate ( [ 0.0, yd, 0.0 ] ) tool_cutter_fastener ( z_offset );
        }
    }
    ...
    if ( fastener_column_count == 2 )
    {
        ...
        {  
            ...
            if ( col == 0 )
            {
                m = ( n2_col1 + n2_col2 > fastener_count ) ? 1 : 0;
                n = n2_col1 - m;
                ...
                {   
                    ...
                    if ( ( y > 0 ) && ( y < n ) )
                    {
                        translate ( [ tx, ty, 0.0 ] ) tool_cutter_fastener ( z_offset );
                    }
                }
            }
...
// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------
// Module:      tool_cutter_circular_hinge.
// Module Type: Tool cutter.
//
// Description:
//
// - Cuts a regular hinge into a circular hinge.
// - The origin of the circle is at the center of the entire hinge assembly. So the entire assembly is a perfect circle.
//
// Parameters:
//
// - N/A
...
module tool_cutter_circular_hinge ()
{
    ...
    xs = rs;
    ys = rs;
    zs = 2.0*( m_leaf_gauge + c );
    
    color ( m_component_color )
    difference ()
    {
        scale ( [ xs, ys, zs ] ) cube ( size = 1.0, center = true );
        cylinder ( r = m_leaf_height / 2.0, h = zs + 2.0*c, center = true );
    }
}

// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------
// Module:      tool_cutter_semi_circular_leaves.
// Module Type: Tool cutter.
//
// Description:
//
// - Cuts the hinge leaves into semi-circles, but leaves the knuckle joint untouched.
// - The origin of each semi-circle, is offset from the center for the assembly, and located at the edge
//   of the knuckle joint cylinder.
//
// Parameters:
//
// - N/A
...
{
    c  = 1.0;        
    xs = m_leaf_height + 2.0*m_leaf_gauge + c;
    ys = m_leaf_height + c;
    zs = 2.0*( m_leaf_gauge + c );
    ...
    difference()
    {
        ...
        union ()
        {
            scale ( [ 2.0*m_leaf_gauge, m_leaf_height+c, zs + 2.0*c ] ) cube ( size = 1.0, center = true );
            translate ( [  m_leaf_gauge, 0.0, 0.0 ] ) cylinder ( r = m_leaf_height / 2.0, h = zs + 2.0*c, center = true );
            translate ( [ -m_leaf_gauge, 0.0, 0.0 ] ) cylinder ( r = m_leaf_height / 2.0, h = zs + 2.0*c, center = true );
        }
    }
}
...
module tool_cutter_pin_shaft_counterbore ( diameter_top, depth_top, shape_top, diameter_bottom, depth_bottom, shape_bottom )
{
    ...
    // Compute diamter of polygon, inscribed by a circle of radius d/2.
    ...
    if ( BOTTOM_COUNTER_BORE_ENABLED )
    {
        ...
        {   
            translate ( [ xd, yd0, zd ] ) rotate ( [ rx, ry, rz ] ) cylinder ( h = h0, r = r0, center = true, $fn = s );        
        }
    }
