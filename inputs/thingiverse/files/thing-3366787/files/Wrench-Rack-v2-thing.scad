// A space-separated list of wrench sizes. This may be
// metric values (i.e. "6.5 9 10 12 14") or fractional SAE values
// (i.e. "3/4 13/16 7/8 1 1-1/8 1-1/4"), or any combination of
// either. A value less than 3 is assumed to be inches.
// Order doesn't strictly matter; I just prefer ascending.
wrenches = "5.5 1/4 10 12 22 1-1/2";

// Spacing of the posts. This is twice the thickness of
// the ring on the box end (of the largest in the set),
// plus some wiggle room to get wrenches on and off the
// post. My standard box wrenches might be 5mm thick,
// while the ratchet wrenches are nearly
// 10mm. Wider spacing might be needed for SAE labels,
// or if the hanging ends of the wrenches get in the way
// of each other.
spacing = 11; // [10:30]

// Thickness of the wrench box, which will be the post height.
wrencht = 13; // [5:20]

// Angle of the box head relative to the handle. The posts
// tilt back/up this much so the handle is parallel to the base.
posta = 10; // [0,5,10,15,20,25]

// Thickness of the base plate
baset = 4; // [2,3,4,5]

// Approximate left/right margin. If you plan to gang them next
// to each other then this should be half the spacing.
hmargin = 6;	// [0:15]

// Top/bottom margin.
vmargin = 1;	// [0:10]

// Attachment screw diameter. A screw hole is placed at
// either end between the first and last two posts. If you're
// creating a single-post mount you'll have to set this to zero
// and make your own holes.
screwd = 5;		// [0,4,5,6]

// Fudge the hex by this factor. Basically, we want to create
// a circular post the wrench will fit around. On my wrenches,
// the circle that fits into the inside radius of the
// teeth is 90% of that of the outside.
undersizefactor = 0.9;

// Taper the hex post from top to bottom, making is easier
// to slide the wrench on the post
taper = 1; // [0,1]
...
// Text height
texth = 6;	// [0,5,6,7,8,9,10]

// Text thickness
textt = 0.8;	// [0.4,0.6,0.8,1]

//----------------------------------------------------
// Mastercraft ratchet wrench sets
//wrenches = "5/16 7/16 9/16 11/16";
//wrenches = "9 10 12 14";
//spacing = 22;
//wrencht = 12;
//posta = 20;
...
//wrenches = "1/4 5/16 11/32 3/8 7/16 1/2 9/16";
//wrenches = "5/8 11/16 3/4 13/16 7/8";
//wrenches = "7 8 9 10 11 12 13 14";
//wrenches = "15 17 18 19 21 22";

// Stanley wrenches
//wrenches = "3/8 7/16 1/2 9/16 5/8 11/16 3/4";

//----------------------------------------------------
function hexdia(w) = w*(2/sqrt(3));
function wrench2hex(x) = hexdia((x>3) ? x : x*25.4);

// String parsing courtesy of the most excellent
// https://www.thingiverse.com/thing:526023

// Parse x or x.y
function parsedec(f) =
	let( v = split(f,".") ) len(v[1])
		? (parse_int(v[0]) + parse_int(v[1])/10)
		: parse_int(v[0]);

// Parse x or x/y or x.y
function parsefraction(f) =
	let( v = split(f,"/") ) len(v[1])
		? (parse_int(v[0]) / parse_int(v[1]))
		: parsedec(v[0]);
		
// Parse x, x/y or x-y/z or x.y
function parsewholefraction(f) =
	let( v = split(f,"-") ) len(v[1])
		? (parse_int(v[0]) + parsefraction(v[1]))
		: parsefraction(v[0]);

// Two arrays; wrenchtext is the labels, postsize is the
// OD of the hex posts.
// Generating postsize is where the real magic for this
// design happens... convert each label string to a
// numeric value, adjusted to metric units, and then
// calculate the hex head diameter. The rest is layout.
wrenchtext = split(wrenches," ");
postsize =
	[for(w=wrenchtext) wrench2hex(parsewholefraction(w))];

//----------------------------------------------------
module post(wd) {
	pd = wd * undersizefactor;
	bo = (pd/2)*sin(posta);
	translate([0,0,baset+bo])
		rotate([-posta,0,0])
			cylinder(d1=pd,d2=pd-taper,h=wrencht);
	...
	if( posta ) {
		translate([0,0,baset])
			hull() {
				cylinder(d=pd,h=0.1);
				translate([0,0,bo])
				rotate([-posta,0,0]) cylinder(d=pd,h=0.1);
			}
	}
}
module label(t) {
	linear_extrude(height=textt)
		text(t,size=texth,font=textfont,halign="center");
}

function sumv(v,i) = (i ? v[i] + sumv(v,i-1) : v[i]);

mnd = min(postsize);
mxd = max(postsize);
xl = hmargin*2
	+ (len(postsize)-1)*spacing
	+ sumv(postsize,len(postsize)-1);
bm = max(vmargin,texth+vmargin);
ytl = mxd + vmargin + bm;

difference() {
	union() {
		for(i=[0:len(postsize)-1]) {
			w = postsize[i];
			x = hmargin + i*spacing
				+ sumv(postsize,i) - w/2;
			translate([x,ytl-w/2-vmargin,0]) post(w);
			
			if( texth > 0 ) {
				translate([x,vmargin,baset]) label(wrenchtext[i]);
			}
		}
	...
	if( screwd>0 ) {
		translate([0,ytl-mxd/2,0]) union() {
			hs = hmargin + spacing/2;
			translate([hs+postsize[0],0,0])
				cylinder(d=screwd,h=baset+1);
			translate([xl-hs-postsize[len(postsize)-1],0,0])
				cylinder(d=screwd,h=baset+1);
		}
	}
}
...
//-----------------------------------------------------------
// https://www.thingiverse.com/thing:526023
// include </home/cpb/src/relativity.scad/strings.scad>
