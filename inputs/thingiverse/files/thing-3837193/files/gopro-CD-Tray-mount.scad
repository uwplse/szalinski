...
bar_width=15;
bar_height=3;
outer_finger_length=40;
inner_finger_length=70;

//gopro connector
rotate([-90,0,0])
translate([-gopro_connector_x/2,-gopro_connector_y-0.5,0])
    gopro_connector("triple");

//adapter supports
difference(){
translate([-gopro_connector_x,gopro_connector_x/2,0])
    cube([gopro_connector_x,gopro_connector_x,3]);
translate([-gopro_connector_x,gopro_connector_x*1.5-0.1,0])
    cylinder(h=3,r=gopro_connector_x);
}
...
bar(bar_length, bar_width, bar_height, outer_finger_length, inner_finger_length);

module finger(bar_width, length, width, height, gap) {
    translate([bar_width,(gap/2)-width,0])
        cube([length,width,height]);
    translate([bar_width+length,(gap/2)-width/2,0])
        cylinder(h=height,r=width/2);
}

module bar(length, width, height, outer_finger_length, inner_finger_length) {
    ...
    //fingers
    finger(width, outer_finger_length, width, height, length-width);
    finger(width, outer_finger_length, width, height, -length+width*3);
    finger(width, inner_finger_length, width, height, length/2-width);
    finger(width, inner_finger_length, width, height, -length/2+width*3);
}


//GoPro connector library
/* [Rod and captive nut] */
