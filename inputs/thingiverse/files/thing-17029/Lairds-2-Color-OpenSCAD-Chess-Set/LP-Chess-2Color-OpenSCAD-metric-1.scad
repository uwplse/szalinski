...
module label(l) {
	translate ([size/2,pixel-g,size/2]) rotate ([90,90,0]) scale([1,1,2]) translate([0,0,-halfpixel]) 8bit_char(l,pixel,pixel);
   }
...
module scube(size, center) {
	rotate([45,0,0]) rotate([0,45,0]) cube(size, center=center);
	}

module k_crown(small) {
   sphere(r=small/2);
   translate([-2*small, 0, -small]) scube(small, center=true);
   translate([-small, 0, -2*small]) scube(small, center=true);
   translate([2*small, 0, small]) scube(small, center=true);
   translate([small, 0, 2*small]) scube(small, center=true);
   translate([-2*small, 0, small]) scube(small, center=true);
   translate([-small, 0, 2*small]) scube(small, center=true);
...
module pawn_body(size) {
	difference() {
		cube(size-0.001, center=false);		// body of pawn, trimmed very slightly to avoid overlapping sides
...
module pawn_trim(size) {
   ...
   label("P");
	...
	translate([0,2*small,size*5/6]) cube(small+g);
	translate([small,2*small,size]) cube(small+g);
	translate([small,size,size*5/6]) rotate([45,0,0]) cube(small+g);
	translate([2*small,2*small,size*5/6]) cube(small+g);
	translate([size/2, size/2, size]) sphere(small/2+g, center=true);
	}
...
// don't need this, as it just wastes space (because generated STLs always align
// unless you move/scale them in ReplicatorG. So just merge and print.
...
module printSetDots() {
	if (c == 1) {
		translate ([bl,bf, 0]) cube(pixel);
		translate ([br,bf, 0]) cube(pixel);
		translate ([bl,bb, 0]) cube(pixel);
		translate ([br,bb, 0]) cube(pixel);
		}
	}

module printPieceDots() {
	if (c == 1) {
		translate ([bl,bf, 0]) cube(pixel);
		translate ([br2,bf, 0]) cube(pixel);
		translate ([bl,bb2, 0]) cube(pixel);
		translate ([br2,bb2, 0]) cube(pixel);
		}
	}
