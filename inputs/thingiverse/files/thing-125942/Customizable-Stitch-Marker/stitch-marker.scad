...
linear_extrude(height = thickness) {
	difference() {
		union() {
			...
			hull() {
				translate([radius * 0.9 - width / 2, 0, 0]) circle(r=width / 2);
				translate([radius * 0.9 - width * 3 / 4, radius, 0]) circle(r=width / 4);
			}
		}
		scale([0.9, 1, 1]) circle(r=(needle_size / 2 + clearance) / 0.9);
		translate([radius * 0.9 - width - slot_width, 0, -1]) square([slot_width, radius]);
	}
}
