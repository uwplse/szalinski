...
// Uses text_on
// Source: https://github.com/brodykenrick/text_on_OpenSCAD

// Presets suitable for Little Tikes Count 'n Play™ Cash Register

// "1" Coin

radius = 18;        // Radius of coin
height = 13;        // Height of coin
sink_height = 3;    // Depth of depression in coin
rim_width = 10;     // Width of rim
label="1";          // Text label on coin
ripple_depth=0.5;   // Depth of the bumps on rim (0 for smooth)
...
mSinkRadius1 = ((radius * 2) - rim_width) / 2;
mSinkRadius2 = mSinkRadius1 + sink_height;

difference() {
    union() {
        difference() {
            cylinder(height, radius, radius, false);
            translate([0,0,height - sink_height])
            cylinder(sink_height, mSinkRadius1, mSinkRadius2, false);
        }
        translate([0,0,height - sink_height])
        text_on_circle(t=label, extrusion_height=sink_height, size=radius/1.5);
    }
    if (ripple_depth > 0) {
        translate([0,0,height-ripple_depth])
        union() {
            difference() {
                union() {
                    for(v = [0 : 5 : 360]) {
                      rotate(v)
                      cube([radius*2,1,2],false);
                    }
                }
                translate([0,0,height-ripple_depth-1])
                cylinder(height+ripple_depth+1, radius/2, radius/2, false);
            }
        }
    }
}
