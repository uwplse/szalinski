...
//***********
cutcube_x=max((len(textstring1)+1)*textsize1,(len(textstring2)+1)*textsize2);
cutcube_yz=(textsize1+textsize2)+text_excenter_factor*textsize2;

/*
Sweeping Name Plate with 2 lines
Version I, October 2018
...
module writetext(textstr1, textstr2, sizeit1, sizeit2)
{
        translate([0,sizeit2*distance,0])
            text(textstr1,size=sizeit1,font=fontname,halign="center",valign="center");
        text(textstr2,size=sizeit2,font=fontname2,halign="center",valign="center");
}

module RiseText(textstr1, textstr2, textsize1, textsize2, direction="up") 
{
    rotate([0,-90,0])
    {
        difference()
        {
            intersection()
            {
                rotate_extrude(convexity = 96, $fn = faces)
                    translate([text_excenter_factor*textsize2, 0, 0])
                        rotate([0,0,(direction=="up"?-1:1)*90])
                            writetext(textstr1, textstr2, textsize1, textsize2);
        ...
        if(RoundBase==0)
        {
            translate([(direction=="up"?-1:0)*baseheight, text_excenter_factor*textsize2, 0])
...
RiseText(textstring1, textstring2, textsize1, textsize2,direction);
