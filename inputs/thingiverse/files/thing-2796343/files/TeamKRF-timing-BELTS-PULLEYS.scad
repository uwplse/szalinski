...
/*TeamKRF Edits  8/23/2017  by; Scoo_B_SK8 (a.k.a Charles Finck)

-took the "Parametric Pulley with multiple Belt profiles" and turned into an all-in-one 
that was geared/taylored for ESK8'ing, but can be widely used in many applications
* Belt                 <==Material I Used = TPE and TPU
* Wheel Pulley         <==Material I Used = PLA, HTPLA, CFHTPLA, PETG, and ABS
* MOTOR Pulley         <==Material I Used = PLA, HTPLA, CFHTPLA, PETG, and ABS
*/


/**
 * @name Pulley & TeamKRF's "Scoo_B_SK8 MotorPulley, WheelPulley, & Belt for Electric SkateBoard"
 * @category Printed
 * @using 1 x m3 nut, normal or nyloc
 * @using 1 x m3x10 set screw or 1 x m3x8 grub screw
 * @using 8mm Shaft
 * @using Canninbal Wheels
 */
...
//------------------------------------------------------
//Select what you are making below
MakeBelt        = 1;  // 0 = No, 1 = Yes
MakeWheelPulley = 0;  // 0 = No, 1 = Yes
MakeMotorPulley = 0;  // 0 = No, 1 = Yes   <==OG Setting
//------------------------------------------------------
BeltWidth       = 15; //in "mm"
BeltThickness   = 4;  //in "mm"

Guide_wall      = 0;  // 0 = No, 1 = Yes
GuideWALLhieght = 7;  //added additional belt guidewall

teeth = 59;			// Number of teeth, standard Mendel T5 belt = 8,
...
BELT_type = "T5-Mod"; // <==MANUALLY enter pulley/belt type from above for label

wheel_shaft = 20;   // TeamKRF ESK8 WheelShaft diameter=20
motor_shaft = 11;	// NEMA17 motor shaft exact diameter = 5   
                    // also =  [TeamKRF ESK8 (motorshaft diameter=11)]

pulley_t_ht = 20;	// length of toothed part of pulley, standard = 12 

rP1=2.25;           // Wheel HUB screw post radius
H1=22;              // Wheel HUB screw post length/depth

pulley_b_ht = 3;	// pulley base height, standard = 8. 
...
///////////////////////////////////////////////////////////////
//OG NUTS n Stuff      TeamKRF... I dont use these...but you can.
add_OGnutsBelow = 0; //0 = No, 1 = Yes  //TeaKRF
...
if (MakeMotorPulley || MakeWheelPulley == 1){    
echo (str("****************************************************************")); 
echo (str(" ")); 
echo (str("Belt type               = ",belt_type));
echo (str("Number of teeth         = ",teeth));
echo (str("Pulley Outside Diameter = ",pulley_OD,"mm"));     
echo (str("Pulley Hiegth           = ",(pulley_b_ht + pulley_t_ht + retainer_ht),"mm"));
echo (str("GuideWall Diameter      = ",(pulley_OD + GuideWALLhieght),"mm"));
//TeamKRF     link below helps with belt size
echo (str(" "));
echo (str("Use link below to find belt length"));
echo (str("https://www.bbman.com/belt-length-calculator/"));     
echo (str(" "));       
echo (str("*****************************************************************"));
...
//TeamKRF
if (Guide_wall == 1){
if (MakeMotorPulley == 1 || MakeWheelPulley == 1){
translate([0,0,(pulley_b_ht + pulley_t_ht)]){
GuideWALL();
}
...
///////////////////////////////////////////////// 
//TeamKRF 
if (MakeWheelPulley == 1){
translate([0,-5,5]){WheelHUBposts();}
}
/////////////////////////////////////////////////

//hole for motor shaft
if (MakeMotorPulley == 1){
translate([0,0,-1]){
cylinder(r=motor_shaft/2,h=pulley_b_ht + pulley_t_ht + retainer_ht + 2,$fn=motor_shaft*4);
}
translate([0,0,18]){
cylinder(r=12.75,h=pulley_b_ht + pulley_t_ht + retainer_ht + 2,$fn=motor_shaft*4);
}
}
//hole for Wheel shaft
if (MakeWheelPulley == 1){
translate([0,0,-1]){
cylinder(r=wheel_shaft/2,h=pulley_b_ht + pulley_t_ht + retainer_ht + 2,$fn=wheel_shaft*4);
}
translate([0,0,15]){
cylinder(r=(wheel_shaft/2)+((pulley_OD/2)-15),h=pulley_b_ht + pulley_t_ht -5 + retainer_ht,$fn=wheel_shaft*4);
}
}	
//captive nut and grub screw holes
if(add_OGnutsBelow == 1){   //TeamKRF  these are OG captive nuts n grubs		
if ( pulley_b_ht < m3_nut_flats ) { echo ("CAN'T DRAW CAPTIVE NUTS, HEIGHT LESS THAN NUT DIAMETER!!!"); 
} 
else {
...
union(){
//entrance
translate([0,-pulley_b_ht/4-0.5,motor_shaft/2+m3_nut_depth/2+nut_shaft_distance])
cube([m3_nut_flats,pulley_b_ht/2+1,m3_nut_depth],center=true);	
//nut
if ( m3_nut_hex > 0 ){
// hex nut
translate([0,0.25,motor_shaft/2+m3_nut_depth/2+nut_shaft_distance]) 
rotate([0,0,30]) 
cylinder(r=m3_nut_points/2,h=m3_nut_depth,center=true,$fn=6);
} 
else {
// square nut
translate([0,0.25,motor_shaft/2+m3_nut_depth/2+nut_shaft_distance]) 
cube([m3_nut_flats,m3_nut_flats,m3_nut_depth],center=true);
}	
//grub screw hole
rotate([0,0,22.5])
cylinder(r=m3_dia/2,h=pulley_b_dia/2+1,$fn=8);   
}
}
}
}	
...
rotate([0,0,180])
translate([8,16,0]){
mirror([1,0,0])text(text = str(BELT_type),size=4);
}
linear_extrude(height=2)
rotate([0,0,180])
translate([6,-18,0]){
mirror([1,0,0])text(text = str("T#",teeth),size=4);
}
}
if (MakeWheelPulley == 1){
linear_extrude(height=1)
rotate([0,0,180])
translate([8,((pulley_OD/2) - 4),0]){
mirror([1,0,0])text(text = str(BELT_type),size=4);
}
linear_extrude(height=1)
rotate([0,0,180])
translate([6,((pulley_OD/2) * -1),0]){
mirror([1,0,0])text(text = str("T#",teeth),size=4);
}
}
}
...
echo (str("Belt type             = ",belt_type));
echo (str("Number of teeth = ",teeth)); 
echo (str("Belt Width           = ",(BeltWidth),"mm ")); 
echo (str("Belt Thickness     = ",(BeltThickness),"mm "));          
echo (str("Belt Length          = ",6.28*((pulley_OD-tooth_depth)/2),"mm    <==========="));
echo (str(" "));
echo (str("Use link below to find belt length"));
echo (str("https://www.bbman.com/belt-length-calculator/"));     
echo (str(" "));   
...
/////////////////SCREW POSTS////////////////////////////////////////
module WheelHUBposts(){
translate([0,20,-13]){
cylinder(r=rP1,H1+10); 
}
translate([0,20,8]){
cylinder(r=rP1+4,H1);
}
...
translate([0,0,6]){  
difference(){
cylinder(r=((teeth*1.54)/2)+BeltThickness,BeltWidth-1);  //1.54=HTD_5 ratio    
}
}
}
/////////////////////////Guidewall//////////////////////////////////
module GuideWALL($fn=100){      //
difference(){
translate([0,0,0]){ 
cylinder(r=((teeth*1.54)/2) + GuideWALLhieght,3); 
}
translate([0,0,0]){ 
cylinder(r=((teeth*1.54)/2)-2,3); 
}
