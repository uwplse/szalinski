...
/* [Hidden] */
pi = 3.14159;	// pi
rad = 57.29578; // Radiant in Grad
$fn = 96;		// OpenSCAD-Variable: Anzahl der Sehnen zum Erzeugen eines Kreises
...
module stirnrad(modul, zahnzahl, breite, bohrung, eingriffswinkel = 20, schraegungswinkel = 0, optimiert = true) {
	...
	union(){
		...
		if (optimiert) {
			linear_extrude(height = breite){							// Zentralbohrung mit "Huelse"
				difference(){
						circle(r = (bohrung+r_loch)/2);
						circle(r = bohrung/2);							// Zentralbohrung
					}
				}
