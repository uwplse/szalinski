// fob.scad
// library for parametric Label, Tag, Fob, Keychain, ID, Marker, Plaque
...
$fn=50;
TZT_Msg=Message;
TZT_Width=Label_Width;
TZT_Height=Label_Height;
TZT_Fnt=Font_Name;
TZT_Fsz=Font_Size;
TZT_Hsz=TZT_Height/2*Hole_Adjustment/100;

TZT_PLATE ();
linear_extrude(height = 1.5) {
    translate ([TZT_Height/3,0,0])
    text(TZT_Msg,size=TZT_Fsz,font=TZT_Fnt,halign="center",valign="center");
}
...
module TZT_PLATE () {
    difference () {
        union () {
            difference () {
                union () {
                    cube ([TZT_Width,TZT_Height,3],true);
                    for (TZT_i=[-2:4:2]) {
                        translate ([TZT_Width/TZT_i,0,0]) {
                            cylinder (3,TZT_Height/2,TZT_Height/2,true);
                        }
                    }
                }
                translate ([0,0,1.5]) {
                    union () {
                        cube ([TZT_Width,TZT_Height-3,3],true);
                        for (TZT_i=[-2:4:2]) {
                            translate ([TZT_Width/TZT_i,0,0]) {
                                cylinder (3,TZT_Height/2-1.5,TZT_Height/2-1.5,true);
                            }
                        }
                    }
                }
            }
            translate ([TZT_Width/-2,0,0]) {
                cylinder (3,TZT_Height/2,TZT_Height/2,true);
            }
        }
        translate ([TZT_Width/-2,0,0]) {
            cylinder (4,TZT_Hsz-1.5,TZT_Hsz-1.5,true);
        }
    }
}
