...
n1 = 14; //red gear number of teeth
n2 = 35; //green gear
mm_per_tooth = 8.6; //all meshing gears need the same mm_per_tooth (and the same pressure_angle)
thickness    = 5;
hole         = 5.6;
hole2        = 8.6;
height       = 11;
...
{ union(){
    ...
    difference() 
    {
        cylinder(r=12,h=8,center=true,$fn=90);
        cylinder(r=hole/2,h=10,center=true,$fn=80);
    translate([0,6,0]) cube([6,3,100],center=true);
    translate([0,15,0.5]) rotate([90,0,0]) cylinder(r=1.6,h=15,$fn=20);
        
    }
}
}
translate([ 0,  d12+2, 0]) rotate([0,0,-($t+n2/2-0*n1+1/2)*360/n2]) color([0.75,1.00,0.75]) 
{ difference() {
    union() {
    gear(mm_per_tooth,n2,thickness,hole2,0,108);
    translate([0,0,5.5]) 
    difference() 
    {
        cylinder(r=15,h=8,center=true,$fn=90);
        cylinder(r=hole2/2,h=10,center=true,$fn=80);
translate([0,0,2]) cylinder(r=7.9,h=8,center=true,$fn=6);
       
    }
}
 for (a =[0:5]){
     rotate([0,0,a*360/6]) translate([0,28,0]) cylinder(r=8,h=30,center=true,$fn=50);
    }
