...
// hex shaft and hub hole support added by Tyler Bletsch, January 2019
...
E = 0.001*1; // tiny value to nudge exactly-equal difference() issues away
...
// NOTE: If added a hex shaft, set this smaller to the hex size (or to zero)
motor_shaft = 5.2;	
// 3mm hole diameter
m3_dia = 3.2;		
// Select nut type
...
// enable hex shaft hole?
shaft_hex = true; // add a hex shaft

// size of hex hole across the flats in mm (12.954 is 0.5in with 2% margin)
shaft_hex_size = 12.954; // mm

// size of an optional chamfer (beveled edge) in mm
shaft_hex_chamfer = 1.0; // mm

/* HUB HOLES SUPPORT */

// Enable an array of holes on the pully (size permitting)?
hub_holes = true;

// Distance from center to put the holes (47.625mm (1.875") is the Vex VersaHub distance)
hub_holes_circle_dia = 47.625;

// Number of holes to drill (6 is the Vex VersaHub number)
hub_holes_count=6;

// Diameter of holes drilled in mm (4.4958mm (0.177") is the free fit diameter for a #8-32 screw, which is the Vex VersaHub size)
hub_holes_dia = 4.4958; 

// Minimum distance from hole to outer diameter of the pulley in mm
hub_hole_min_margin=6;
...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
	...
	difference()
	 {	 
        ...
        if (shaft_hex) {
            shaft_hex_dia = shaft_hex_size*2/sqrt(3); // we use a 6-point circle for our hex; this math converts flat-to-flat size to point-to-point diameter.
            translate([0,0,-idler_ht-E]) cylinder($fn=6,d=shaft_hex_dia,h=pulley_b_ht + pulley_t_ht + retainer_ht + idler_ht + 2*E);
            if (shaft_hex_chamfer) {
                translate([0,0,pulley_b_ht+pulley_t_ht+retainer_ht-shaft_hex_chamfer]) cylinder($fn=6,d2=shaft_hex_dia+shaft_hex_chamfer*2,d1=shaft_hex_dia,h=shaft_hex_chamfer+E);
                translate([0,0,-idler_ht-E]) cylinder($fn=6,d1=shaft_hex_dia+shaft_hex_chamfer*2,d2=shaft_hex_dia,h=shaft_hex_chamfer+2*E);
            }
        }
        
        if (hub_holes) {
            if (hub_holes_circle_dia+hub_holes_dia <= pulley_OD-hub_hole_min_margin) {
                for(j=[1:hub_holes_count]) rotate([0,0,j*360/hub_holes_count])
			translate([hub_holes_circle_dia/2,0,-idler_ht-E]) #cylinder(d=hub_holes_dia,h=pulley_b_ht + pulley_t_ht + retainer_ht + idler_ht + 2*E,$fn=20);
	
            } else {
                echo("Pulley too small for specified hub holes! (OD=",pulley_OD,", hub_holes_circle_dia+hub_holes_dia=",hub_holes_circle_dia+hub_holes_dia);
            }
        }
        
        
        // x-ray cutaway to check geometry
        //translate([0,-40,6]) cube([40,80,40]);
