...
//Updated 5/10/17: Updated to add non-hardhat options for DeGrazendeSlak (Eloy Hensen). There should be a new tab to add a button hole and counterweight to the name tag for use in coverall pocket snap type buttons. I also fixed the CatchThemAll = 0 error.
...
PostShift = 2; //[0:.1:5]
//Use BarY and BarZ to modify the text bar at the bottom of the characters.
BarY = 1.5; //[1:0.2:4]
BarZ = 1.5; //[1:0.2:4]
...
/* [Non Hardhat Options] */
//Change to diameter >5.0 to cut a hole in the tab. (Hi DeGrazendeSlak!)
buttonhole = 4.9; //[4.9:0.1:15]
//This will add a counterweight to the bottom of the tab.
buttonweight = "No"; //[No, Yes]
//This will change the width (X) of the tab. (Default is 28)
tabX = 28; //[20:0.1:40]
//This will change the length (Y) of the tab. The button hole is centered in the Y. (Default is 16)
tabY = 16; // [15:0.2:30]
...
//difference(){
//    hull() {
//        translate([0,FontSize*1.5+(10+12/2)-7,0])cylinder(d=17,h=tabThick,$fn=30);
//        intersection(){ 
//            translate([-(tabX/2),(FontSize*LineSpace)*0.55,0]) cube([tabX,FontSize*(LineSpace),1]);
//            //recreate lower text
//            translate([0,0,0])linear_extrude(height=FontExtrude, convexity=10)
//            text(line1,size=FontSize,font=FontName,halign="center",valign="baseline",spacing=FontSpace);
...
//        }
//    }
//    translate([0,FontSize*1.5,0]){
//        translate([0,0,-0.1])cylinder(d=12,h=5,$fn=30);
//        translate([0,(12+10)/2-1-7,5/2-0.1])cube([2,12,5],center=true);
//        translate([0,10+12/2-7,0.75])cylinder(d=12,h=2,$fn=30);
//        translate([0,10+12/2-7,-0.1])cylinder(d=2,h=2,$fn=30);
//    }
//}
...
//
//Add Text. Line 1 at bottom and Line 4 at the Top
module TextClip (){
    union(){
        //TAB();
        WriteText(line1);
...
module BaseBar(linetext,linenum,lowertext=""){ //Pass the text for line 1 and autogenerate bar
    hull(){ 
        intersection(){
            ...
            translate([-(len(linetext)/2)*FontSize*FontSpace*1.25,0,0]) cube([len(linetext)*FontSize*FontSpace*1.5,BarY,BarZ]);
        }
    }
    if(linenum>1){
        hull() {
            intersection(){ //Top of left post
                ...
                translate([-(PostShift+FontSize*FontSpace/2),-(FontSize*LineSpace),0]) cube([BarY,FontSize*(LineSpace)*2,BarZ]);
            }
            intersection(){ //Bottom of left post
                ...
                //bound box to skip the lower edges of characters
                translate([-FontSize*FontSpace,-(FontSize*LineSpace)/2,0])cube([FontSize*FontSpace*3,FontSize*LineSpace*.5,BarZ]);
            }
        }
        hull(){
...
module TAB (){
    rotate([0,0,180]) difference(){
        hull(){ //outer shell
            ...
            translate([((tabX/2)-rad),tabY+tabClip-rad,0]) cylinder(h=tabZ, r=rad, $fn=25);
            if(buttonweight=="Yes"){
                translate([-tabX/2,tabY,0])cube([tabX,20,4]);
            }
                
        }
        translate([-(tabX*1.1)/2,-tabHead-.1,tabThick]) cube(size=[tabX*1.1,tabY+tabHead+.1,tabZ]);
        if(buttonhole>4.9){
            translate([0,tabY/2,-0.1])cylinder(d=buttonhole,h=tabZ+1,$fn=100);
        }
        if(buttonweight=="No"){
