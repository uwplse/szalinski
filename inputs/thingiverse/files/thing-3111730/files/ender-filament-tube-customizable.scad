...
body_length = 46;
body_diameter = 26;
cap_diameter = body_diameter + 4;
cap_height = 2.5;
top_screw = 1; // [1:Yes,0:No]
tolerance = 0.15;
...
module capScrew() {
    metric_thread (diameter=30, pitch=1.5, length=screw_height);    
}
...
difference() {
    union() {
        ...
        cylinder(r1=30/2-tolerance, r2=32/2, h=2);

        translate([0, 0, screw_height + 2])
        cylinder(r=body_diameter / 2, h=body_length);

        translate([0, 0, screw_height + 2 + body_length])
        cylinder(r1=body_diameter/2-tolerance, r2=cap_diameter/2, h=cap_height);
        
        if (top_screw) {
            translate([0, 0, screw_height + 2 + body_length + cap_height])
            capScrew();
        }
    }
    
    translate([0, 0, -epsilon])
    cylinder(r=(body_diameter - 10) / 2, h=screw_height + 2 + body_length + cap_height + screw_height + epsilon * 2);
}


/// Source: http://dkprojects.net/openscad-threads/threads.scad
