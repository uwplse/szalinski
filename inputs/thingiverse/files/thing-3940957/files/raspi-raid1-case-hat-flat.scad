...
// hat: +35 height
 ...
 DiskWallY = DiskWidth + BoxThickness + 1;
 FullBoxWidth = DiskWallY + PiBoxWidth + BoxThickness;
 ...
 Disk1Position = [3, BoxThickness+0.5, FirstDiskHeight];
 Disk2Position = [3, BoxThickness+0.5, SecondDiskHeight];
 ...
 // raspberry pi specs
 RaspberryPiBplusX = 5;
 RaspberryPiBplusPillarX = RaspberryPiBplusHolesDistToSide + BoxThickness+RaspberryPiBplusX;
 RaspberryPiBplusPillars = [
  [
    [RaspberryPiBplusPillarX, 
      FullBoxWidth-BoxThickness-RaspberryPiBplusHolesDistToSide],
    90, RaspberryPiBplusHolesDistToSide
  ],
  [
    [RaspberryPiBplusPillarX+RaspberryPiBplusBetweenHolesLength,
      FullBoxWidth-BoxThickness-RaspberryPiBplusHolesDistToSide],
    90, RaspberryPiBplusHolesDistToSide
  ],
  [
    [RaspberryPiBplusPillarX, 
      FullBoxWidth-BoxThickness-RaspberryPiBplusHolesDistToSide-RaspberryPiBplusBetweenHolesWidth],
      -90, RaspberryPiBplusHolesDistToSide+PiBoxEmptySpace-0.5
  ],
  [
    [RaspberryPiBplusPillarX+RaspberryPiBplusBetweenHolesLength,
      FullBoxWidth-BoxThickness-RaspberryPiBplusHolesDistToSide-RaspberryPiBplusBetweenHolesWidth],
 ...
 RaspberryPiBplusPillarHeight = 20;
...
// hat: add power button
 ...
 // one hole for disk
 module bolthole(h, d) {
   translate([0,h/2+1,0]) rotate([-90, 0, 0])
     cylinder(h=h+2, d=d, center=true, $fn = 10);    
 }
 ...
 module draw_pi() {
  translate([BoxThickness+RaspberryPiBplusX,  
   FullBoxWidth+0.8,
   BoxThickness+RaspberryPiBplusPillarHeight+RaspberryPiBplusThickness+RaspberryPiBplusZoffset])
  color("orange")
  rotate([-180,0,0])
...
module fillet(l, r, pos, rot=[0,0,0]) {
 ...
 module pillar(p, dir, rw) {
  translate([p.x, p.y-0.2,BoxThickness-0.1]) 
   difference() {
    union() {
      cylinder(h=RaspberryPiBplusPillarHeight, d=RaspberryPiBplusHoleOuterSupportDiameter, $fn=20);
      round_fillet(RaspberryPiBplusHoleOuterSupportDiameter);
      echo(dir, rw);
      #rib(dir, rw);
    }
    cylinder(h=RaspberryPiBplusPillarHeight+1, d=RaspberryPiBplusScrewDiameter+1, $fn=20);
    }
}

module rib(dir, w) {
  rotate([0,0,dir])
    translate([RaspberryPiBplusScrewDiameter/2+0.2, -BoxThickness, 0])
      cube([w, BoxThickness*2, RaspberryPiBplusPillarHeight*0.8]);
}

module round_fillet(d) {
  r=d/2;
  r1=1.75*r;
/*  difference() {
    cylinder(h=r1-r,r=r1, $fn=20);
    translate([0,0,r1-r])
      rotate_extrude(convexity = 10)
      translate([r1, 0, 0])
      circle(r = r1-r, $fn = 100);
  }*/
  cylinder(r1-r, r1, r); 
}
...
module carve_holes() {
    ...
    for (foot=Feets) // Feets holes
       translate([foot[0], foot[1], -1]) cylinder(h=BoxThickness+2, d=FootSmallestDiameter + 0.25);

// fan grid
    translate([(RaspberryPiBplusLength-FanSize)/2+2*BoxThickness, FullBoxWidth-RaspberryPiBplusWidth/2-2*BoxThickness, 0]) 
  fangrid(FanSize);

  e = BoxThickness+RaspberryPiBplusX;

// power socket
    h=[RaspberryPiBplusPowerSocketDistToBoard,
       RaspberryPiBplusHatPowerSocketDistToBoard];
    for (i=[0:1]) 
      translate ([RaspberryPiBplusPowerSocketDistToSide+e-RaspberryPiBplusPowerSocketWidth/2,FullBoxWidth-BoxThickness-1, RaspberryPiBplusPillarHeight+BoxThickness-RaspberryPiBplusPowerSocketHeight-h[i]+0.5]) cube([RaspberryPiBplusPowerSocketWidth,2*BoxThickness,RaspberryPiBplusPowerSocketHeight]);

// power button
    translate ([RaspberryPiBplusPowerButtonDistToSide+e-RaspberryPiBplusPowerButtonWidth/2,FullBoxWidth-BoxThickness-1, RaspberryPiBplusPillarHeight+BoxThickness-RaspberryPiBplusPowerButtonHeight-RaspberryPiBplusPowerButtonDistToBoard+RaspberryPiBplusPowerButtonHeight+0.5]) cube([RaspberryPiBplusPowerButtonWidth,2*BoxThickness,RaspberryPiBplusPowerButtonHeight]);
    
// hdmi socket
    translate ([RaspberryPiBplusHDMISocketDistToSide+e-RaspberryPiBplusHDMISocketWidth/2,FullBoxWidth-BoxThickness-1, RaspberryPiBplusPillarHeight+BoxThickness-RaspberryPiBplusHDMISocketHeight-RaspberryPiBplusHDMISocketDistToBoard+0.5]) cube([RaspberryPiBplusHDMISocketWidth,2*BoxThickness,RaspberryPiBplusHDMISocketHeight]);

// sound socket
    translate ([RaspberryPiBplusSoundSocketDistToSide+e,FullBoxWidth-BoxThickness-1, RaspberryPiBplusPillarHeight+BoxThickness-RaspberryPiBplusSoundSocketDiameter/2-RaspberryPiBplusSoundSocketDistToBoard+1])
      rotate([-90,0,0]) cylinder(h=2*BoxThickness,d=RaspberryPiBplusSoundSocketDiameter, $fn=20);

// led hole
  translate ([-1,FullBoxWidth-e-RaspberryPiBplusLedHoleDistToSide, RaspberryPiBplusPillarHeight+BoxThickness-RaspberryPiBplusLedHoleHeight-RaspberryPiBplusLedHoleDistToBoard+0.5])
      cube([2*BoxThickness,RaspberryPiBplusLedHoleWidth,RaspberryPiBplusLedHoleHeight]);

// power led hole
    translate ([-1,FullBoxWidth-e-RaspberryPiBplusPowerLedHoleDistToSide, RaspberryPiBplusPillarHeight+BoxThickness-RaspberryPiBplusPowerLedHoleHeight-RaspberryPiBplusPowerLedHoleDistToBoard+0.5])
...
// disk wall
      translate ([BoxThickness-0.1, DiskWallY, 0.1]) cube([DiskWallLength, BoxThickness, DiskWallHeight]);
// transversal beam
...
// internal fillets
      ...
      fillet(BoxHeight - BoxThickness, FilletR, [BoxThickness,DiskWallY,DiskWallHeight], [0,90,-90]);
      fillet(BoxHeight - BoxThickness, FilletR, [BoxThickness,DiskWallY+BoxThickness,DiskWallHeight], [0,90,0]);
// pi pillars
      for (i=RaspberryPiBplusPillars) pillar(i[0], i[1], i[2]);
// text in front
