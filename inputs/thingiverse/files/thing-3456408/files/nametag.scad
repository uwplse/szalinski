//font_name = "NanumSquareRound ExtraBold";
...
//Set the font style - but it might be your desired style is not available for the selected font. Just try it and see.
font_style = "Bold";//[Regular,Bold,Italic,Bold Italic,Narrow,ExtraBold,Black,Light,Thin,Medium Italic, SemiBold,Condensed,Expanded,Oblique]
...
y = [0, 1, 0];
z = [0, 0, 1];
full_font = str(font_name, ":style=", font_style);


module tri_column(r=15, R=10, h=100)
{
    hull() for (rot = [60, 180, 300])
    rotate([0, 0, rot]) translate([r, 0, 0]) cylinder(r=R, h=h, center=true);
}
...
module main()
{
    ...
    difference()
    {
        tri_column(r=8, R=5, h=h, $fn=72);
        tri_column(r=3, R=3, h=h+0.1);
        ...
        for(i = [0:2])
        {
            ...
            text(txts[i], font=full_font, valign="center", halign="center");
        }
        
        translate((+h/2-4)*z) rotate(90*y) cylinder(d=4, h=40, $fn=24);
        translate((-h/2+4)*z) rotate(90*y) cylinder(d=4, h=40, $fn=24);
    }
}
