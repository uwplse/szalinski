...
part = "all"; // [all:Everything, base: Base Only, nuts:Nuts Only, arm:Upper Arm Only]

//Length of the base feet
foot_length = 200;
...
{
    rotate([0,0,-25])
    translate([foot_length/2,0,2.5])cube([foot_length,5,5], center=true);
    rotate([0,0,25])
    translate([foot_length/2,0,2.5])cube([foot_length,5,5], center=true);
...
module toroid()
{
rotate_extrude(angle = 360, convexity = 10)translate([10,0,0])circle(r=3);
}

module toroid_with_gap()
{
    difference(){toroid();translate([-20,4,-5])cube([20,2,10]);};
}
...
module feeder_arm()
{
    ...
    rotate([180,180,0]){
        ...
        washer();
        translate([-arm_length/2 - 6,0,2.5])
        cube([arm_length, 5, 5], center=true);
        translate([-6,0,5])
        rotate([0,-95,0])
        rotate([0,0,45])
        cylinder(30,7,3, $fn=4);
        translate([-arm_length-15,0,3])
        toroid_with_gap();
    }
}
