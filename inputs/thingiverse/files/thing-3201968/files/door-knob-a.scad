// door_knob_a.scad
// library for parametric door pull, knob, handle - Style A
// Author: Tony Z. Tonchev, Michigan (USA) 
// last update: November 2018


// (in mm)
Knob_Diameter = 40; //[25:80]

//
Knob_Corners = 5; //[3,4,5,6,7,8]

//
Knob_Roundness = 8; //[1:9]
...
// (in degrees)
Character_Rotation = 0; //[0:90]


/* [Hidden] */

$fn=30;
$TZT_Cct=Knob_Corners;
$TZT_Bnt=10-Knob_Roundness;
$TZT_Rad=Knob_Diameter/2;

TZT_Msg=Monogram;
TZT_Fnt=Font_Name;
TZT_Frt=Character_Rotation;

difference () {
    union () {
        TZT_Knob ();
        rotate ([0,0,TZT_Frt]) {
            linear_extrude($TZT_Rad*.2) {
                text(TZT_Msg,size=$TZT_Rad*.7,font=TZT_Fnt,halign="center",valign="center");
            }
        }
    }
    for (TZT_i=[0:120:360]) {
        rotate ([0,0,TZT_i]) {
            translate ([0,0,-$TZT_Rad]) {
                cube ([.7,5,$TZT_Rad*1.5],true);
            }
        }
    }
}
...
module TZT_Knob () {
    union () {
        difference () {
            hull () {
                translate ([0,0,-$TZT_Rad/10]) {
                    cylinder ($TZT_Rad*2,$TZT_Rad/4,$TZT_Rad/4,true);
                }
                for (TZT_i=[0:(360/$TZT_Cct):360]) {
                    rotate ([0,0,TZT_i]) {
                        translate ([$TZT_Rad-$TZT_Rad/$TZT_Bnt,0,0]) {
                            cylinder ($TZT_Rad/4,$TZT_Rad/$TZT_Bnt,$TZT_Rad/$TZT_Bnt,true);
                        }
                    }
                }
            }
            translate ([0,0,$TZT_Rad*2.3]) {
                sphere ($TZT_Rad*2.2);
            }
        } 
        difference(){
            translate ([0,0,-$TZT_Rad*1.2]) {
                TZT_Donut($TZT_Rad/3,$TZT_Rad/4);
            } 
            translate([0,0,-$TZT_Rad*1.15]) {
                TZT_Donut($TZT_Rad/2,$TZT_Rad/4);
            }
            translate([0,0,-$TZT_Rad/1.75]) {
                cylinder ($TZT_Rad,$TZT_Rad,$TZT_Rad,true);
            }
        }    
        translate ([0,0,-$TZT_Rad*1.47]) {
            difference () {
                union () {
                    translate ([0,0,$TZT_Rad/2.1]) {
                        cylinder ($TZT_Rad,$TZT_Rad/4,$TZT_Rad/4,true);
                    }
                    cylinder ($TZT_Rad/10,$TZT_Rad/1.5,$TZT_Rad/1.5,true);
                    translate ([0,0,$TZT_Rad/10]) {
                        cylinder ($TZT_Rad/10,$TZT_Rad/1.5,$TZT_Rad/1.5-$TZT_Rad/10,true);
                    }
                }
            }
        }
...
module TZT_Donut (TZT_Ra,TZT_Rb) {
    rotate_extrude() {
        translate([TZT_Ra, 0, 0])
        circle(TZT_Rb);
    }
}