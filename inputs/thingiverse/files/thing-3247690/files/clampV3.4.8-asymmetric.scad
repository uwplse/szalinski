...
// tweaked for increasing thickness not affecting other parts, renamed dimensions, adjustable knurled finish, made more adjustments available and fixed warnings - 247generator 2018

// this code hack generates an asymmetric clamp for glass print beds
...
clamp_width=30;
clamp_depth=40;
// minimum 10 (halved on bottom)
clamp_thickness=15;
// square is not as strong but fits better
glass_corner_shape=1;//[0:Round,1:Square]
// depth reduction for the glass side.
bottom_reduction=0;
// radius of ball that makes round edges
rounding=2;
// how smoothly rounded or polygonal the rounding is (24 is plenty)
roundedness=8; //[4:4:64]
// adjusts how tight the threads mesh - don't change unless it's too sloppy or tight. (4.3% x 18mm is a 0.774mm gap)
bolt_hole_space_factor=4.3;
/*[bolt]*/
knob_length=10;
knob_diameter=35;
// a lower number makes smaller points: 180 felt good, 250 is sharp but better grip, 400 previews quickly.
knob_knurled_finish=400;
screw_diameter=18;
// keeps the bolt from hanging down too far but limits it to clamping objects bigger this amount
screw_length_reduction=0;
// shows the preview opened by this much:
assembled_screw_position=0;
/*[press]*/
press_diameter=24;
// 2 is zero thickness for press diameter: reduction can help with clearance on shorter clamps with a wide enough screw.
...
screw_length=clamp_length+(clamp_thickness*1.5)-13-(rounding*3.5)-screw_length_reduction;
...
module acorner(tr){
    ...
    echo ("corner outside diameter",tr*2);
    echo ("corner inside diameter",ti*2);
  intersection(){
    difference(){
        // outside corner
      cylinder(r=tr,h=width,$fn=64);
      ...
      translate([0,-rounding-thickness/2,-.1])
        cylinder(r=ti,h=width+.2,$fn=32);
        // make the corner square option
              translate([-(ti/2),-(ti/2)-rounding-thickness/2,(width/2)-.1])
        // for square corner option
        cube([ti*glass_corner_shape,ti*glass_corner_shape,glass_corner_shape*(width+.2)],center=true);
        // remove the remaining inside corner for asym corner
        translate([-ti,(-ti/1.5)-rounding,0]) cube([ti,ti*1.5,width]); 
    }
    translate([-tr,-tr,0])cube([tr,tr,width]); 
  }
}
module base(){
    ...
    minkowski(){
      ...
      ti=thickness*(radius-1);
        ...
        translate([0,-tr,0])cube([length-ti*2,thickness,width]);
            // bottom corner
        translate([length-ti*2,0,0])rotate([0,0,90])acorner(tr);
            // bottom flat
            difference(){
                translate([length-ti+(clamp_thickness/2),0,0])cube([clamp_thickness/2-rounding*2,depth,width]);
                translate([length-ti+(clamp_thickness/8),clamp_depth-rounding-bottom_reduction,0])cube([clamp_thickness,bottom_reduction,width]);
                }
                }
  ...
  }       // hex_countersink_ends(chg=countersink height(thread step/2),cod=thread outer diameter,clf=thread step degrees,crs=resolution,hg=nut height);
