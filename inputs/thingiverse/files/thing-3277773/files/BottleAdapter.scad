// Bottom cylinder height in inches
bottomHeight = .75;

// Top cylinder height in inches
topHeight = .75;

// Cylinder outer diameter in inches
outerDiameter = 1.305;

// Bottom cylinder interior width in inches
bottomInteriorDiameter = 1;

// Bottom threads per inch
bottomThreadsPerInch = 8;

// Top cylinder interior width in inches
topInteriorDiameter = 1.175;

// Top threads per inch
topThreadsPerInch = 7;

difference()
{
    cylinder (d=convert_to_metric(outerDiameter), h=convert_to_metric(bottomHeight));
    english_thread (diameter=bottomInteriorDiameter, threads_per_inch=bottomThreadsPerInch, length=bottomHeight, internal=true);
}

translate([0,0,convert_to_metric(bottomHeight)])
{
    difference()
    {
        cylinder (d=convert_to_metric(outerDiameter), h=convert_to_metric(topHeight));
        english_thread (diameter=topInteriorDiameter, threads_per_inch=topThreadsPerInch, length=topHeight, internal=true);
    }
}

function convert_to_metric(length_in_inches) = length_in_inches * 25.4;
...
{
   // Convert to mm.
   mm_diameter = convert_to_metric(diameter);
   mm_pitch = convert_to_metric(1.0/threads_per_inch);
   mm_length = convert_to_metric(length);
