...
Parts="Everything"; //[Everything, Bottom_only, Lids_only]
...
//lid bigger so we can remove it
bigger=.3;
...
module LidText(textstr, textsize,textheight) 
{   
    linear_extrude(height=textheight,convexity = 32)
        offset(delta=bigger)
            rotate([0,180,0])
    ...
    difference()
    {
        ...
        linear_extrude(height=textheight+lidliplength,convexity = 32)
            offset(delta=-lidIn)
                rotate([0,180,0])
                    writetext(textstr, textsize);

        linear_extrude(height=textheight+lidliplength+1,convexity = 32)
            offset(delta=-(lidIn+lidWidth))
                rotate([0,180,0])
                    writetext(textstr, textsize);
    }
}

if (Parts=="Bottom_only" || Parts=="Everything")
{
    RiseText(textstring,textsize,textheight);
}

if (Parts=="Lids_only" || Parts=="Everything")
{
    translate([0,textsize+2+2*base_radius_add,0])
        LidText(textstring,textsize,1);
}