...
d = 8;            // [4:20]
height = 6;       // [1:20]
// Must small than `d`, affects flow
d_inner = 5;      // [3:20]
// Diameter of central rod, affects flow
d_central = 2.5;  // [0:10]

/* [Advanced] */

// Thickness of the radial walls 
thickness = 0.68;
...
difference() {
  rod(d=d, h=height);
  rod(d=d_inner, h=height+1);
  translated(height/4*z) rotated(22*z) rotated(45*z, n=[0:3]) box([d+1, d/5, d/5]);
}

rotated(45*z, n=[0:3]) box([d_inner, thickness, height]);
rod(d=d_central, h=height);
