...
fullLength = length + wall*2 + endExtra;
fullWidth = thickness + 2*wall;

difference() {
  union() {
    translate ([-fullLength/2, -fullWidth/2, 0]) {
      cube([fullLength, fullWidth, 5], false);
      translate ([0, 0, 5]) {
        cube([5+endExtra+wall, fullWidth, height], false);
      }
    }
    translate ([fullLength/2 - (5+endExtra+wall), -fullWidth/2, 0]) {
      translate ([0, 0, 5]) {
        cube([5+endExtra+wall, fullWidth, height]);
      }
    }
  }
  union() {
    translate ([0, 0, -0.5]) {
      scale(v = [1.1, 1.1, 1]) {
        english_thread(diameter=1/4, threads_per_inch=20, length=6/(25.4));
      }
      cylinder(4.2, 4.2, 0, $fn=40);
    }
    translate ([-length/2, -thickness/2, 5.01]) {
      cube([length, thickness, height+1], false);
    }
  }
}
