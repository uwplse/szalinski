...
if (1) {
    ...
    if (part == "all") {
        ...
        scale([myScale,myScale,1]) translate([52,0,0]) dialTop();
        scale([myScale,myScale,1]) translate([26,21,0]) pin(pinFit);
    }
}
else {
    /* cut-away view */
	...
	union() {
	    dialBottom();
	    rotate([0,0,360/16*8+14.5]) translate([0,0,5.4]) rotate([180,0,0]) dialTop();
...
module dialTop() {
	...
	union() {
	    ...
	    translate([0,12.5,-0.01]) linear_extrude(height=3.02)
		polygon(points = [[-3,5], [-4,11], [4,11], [3,5]],
			paths = [[0,1,2,3,0]]);
	}
...
module maneuver(m) {
    ...
    if (m == "1"){
        rotate([0,0,-3.5])
        translate([-1.3,18.5,1.5]) scale([0.4,0.4,1]) linear_extrude(height=1.5)
        write(m);
    }
    else if (m == "10"){
        rotate([0,0,3])
