...
linear_extrude(height, convexity=10, twist=0,center=true)
union() {
...
translate([0,outer_dia_1/2,0]) difference() {
    ...
    rotate([0,0,90]) wedge(outer_dia_1*2,wedge_angle);
...
translate([0,outer_dia_2/2,0]) difference() {
    ...
    translate([distance,0,0])rotate([0,0,90]) wedge(outer_dia_2*2,wedge_angle);
