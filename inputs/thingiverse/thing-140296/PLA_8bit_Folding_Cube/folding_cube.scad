// 8bit folding cube (for PLA) by whpthomas
// derived from Emmett's thing:135836

// object
object=7;// [0:Plate,1:Ditto Plate,2:Part 1,3:Part 2,4:Part 3,5:Part 4,6:Assembly]
// sub-cube size
L=25;
// Radius of hinge
R=3;
...
module plate() { // MAKE_TARGET
union(){
	if(tabs){
		translate([L,-L,0]) cylinder(r=L*0.9,h=0.15);
		translate([L,L,0]) cylinder(r=L*0.9,h=0.15);
		translate([-L,-L,0]) cylinder(r=L*0.9,h=0.15);
		translate([-L,L,0]) cylinder(r=L*0.9,h=0.15);
	}
...
module bit1() { // MAKE_TARGET
bit();
}

module bit2() { // MAKE_TARGET
mirror([0,1,0])bit();
}

module bit3() { // MAKE_TARGET
bit(R1=[90,90,0]);
}

module bit4() { // MAKE_TARGET
		...
		difference(){
			union(){
				difference(){
					...
					translate([-ll,-ll,ll/2]) cube([lt,lt,lt*2],center=true);
					translate([-ll,0,ll]) cube(lt,center=true);
					translate([0,-ll,ll]) cube(lt,center=true);
				}
				translate([L/2,-L/2,0]){
...
// slit
translate([0,0,(L-tol)*sqrt(3)/4])rotate([0,atan(sqrt(2)),0])rotate([0,0,-45])
	rotate(R1)translate([L/2,-L/2,0])cylinder(r=L/2,h=R/2,center=true);
translate([0,0,-L])cube(2*L,center=true);
translate([0,0,L*1.86])cube(2*L,center=true);
}
...
difference(){
	union(){
		...
		translate([0,0,l/2-(R/8)])sphere(r=(R+t)*3/4, $fn=20);
		support(t);
		mirror([1,1,0])support(t);
	}
	translate(-[0,0,l/2+2*t+(R/8)])sphere(r=(R+t)*3/4, $fn=20);
}}
