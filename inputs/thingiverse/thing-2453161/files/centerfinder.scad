//use <sc.scad> needed to be included due to customizer not being able to import libs
...
D(){intersection(){Cu(w+t,2*(w-t/2),h);U(){Tx(t/2)Cu(w,t,h);
T(w/2,w/2-t/2,0)Cu(t,w,h);
T(w/2,0,0)Rz(45)T(t/2+sharpie,w/2+2*t,0)Cu(t,sqrt(2)*w-t,h);}}
T(0,w/2,h/2)Cu(w,w,h);}
