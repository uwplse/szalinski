...
OpenJsCad.Viewer = function (containerelement, initialdepth) {
  ...
  var shiftControl = $(
    '<div class="shift-scene"><div class="arrow arrow-left" />\
  ...
  $(containerelement)
    ...
    .on("transformend dragstart dragend", function (e) {
      if (
        (e.type == "transformend" && _this.touch.cur == "transforming") ||
  ...
  this.gl.resizeCanvas = function () {
    ...
    if (
      ...
      _this.gl.perspective(
        45,
        _this.gl.canvas.width / _this.gl.canvas.height,
        0.5,
        1000
      );
...
OpenJsCad.Viewer.prototype = {
  ...
  getZoom: function () {
    var coeff =
      (this.viewpointZ - this.ZOOM_MIN) / (this.ZOOM_MAX - this.ZOOM_MIN);
    return coeff;
  },
...
OpenJsCad.Viewer.csgToMeshes = function (initial_csg) {
  ...
  for (var j = 0; j < numpolygons; j++) {
    ...
    var color = [1, 0.4, 1, 1]; // -- default color
...
OpenJsCad.runMainInWorker = function (mainParameters) {
  try {
    if (typeof main != "function")
      throw new Error(
        "Your jscad file should contain a function main() which returns a CSG solid or a CAG area."
      );
    OpenJsCad.log.prevLogTime = Date.now();
    var result = main(mainParameters);
    if (
      typeof result != "object" ||
      (!(result instanceof CSG) && !(result instanceof CAG))
    ) {
      //throw new Error("Your main() function should return a CSG solid or a CAG area.");
    }
    if (result.length) {
      // main() return an array, we consider it a bunch of CSG not intersecting
      var o = result[0];
      if (o instanceof CAG) {
...
OpenJsCad.parseJsCadScriptSync = function (script, mainParameters, debugging) {
  ...
  var _includePath = "examples/"; // anjali
...
OpenJsCad.parseJsCadScriptASync = function (
  script,
  mainParameters,
  options,
  callback
  ...
  var gMemFs = []; // anjali
  for (var i in gMemFs) {
    // let's test all files and check syntax before we do anything
  ...
  for (var fn in gMemFs) {
    ...
    if (!mainFile) mainFile = fn;
    if (fn == "main.jscad" || fn.match(/\/main.jscad$/)) mainFile = fn;
    workerscript +=
      'gMemFs["' +
      gMemFs[fn].name +
      '"] = ' +
      JSON.stringify(gMemFs[fn].source) +
      ";\n";
    ignoreInclude = true;
  }
  if (ignoreInclude) {
  ...
  workerscript +=
    "var _csg_baselibraries=" + JSON.stringify(baselibraries) + ";\n";
  workerscript += "var _csg_libraries=" + JSON.stringify(libraries) + ";\n";
  workerscript +=
    "var _csg_openjscadurl=" + JSON.stringify(openjscadurl) + ";\n";
  workerscript +=
    "var _csg_makeAbsoluteURL=" + OpenJsCad.makeAbsoluteUrl.toString() + ";\n";
  ...
  workerscript +=
    "  OpenJsCad.runMainInWorker(" + JSON.stringify(mainParameters) + ");";
...
";
  ...
  if (!window.Worker)
    throw new Error(
      "Your browser doesn't support Web Workers. Please try the Chrome or Firefox browser instead."
    );
...
OpenJsCad.FileSystemApiErrorHandler = function (fileError, operation) {
  ...
  var errtxt =
    "FileSystem API error: " + operation + " returned error " + errname;
...
OpenJsCad.getParamDefinitions = function (script) {
  ...
  if (scriptisvalid) {
    var script1 =
      "if(typeof(getParameterDefinitions) == 'function') {return getParameterDefinitions();} else {return [];} ";
    script1 += script;
    var f = new Function(script1);
    params = f();
    if (typeof params != "object" || typeof params.length != "number") {
      throw new Error(
        "The getParameterDefinitions() function should return an array with the parameter definitions"
      );
    }
  }
  return params;
...
*/
    ...
    try {
      ...
      this.viewerdiv.innerHTML =
        "<b><br><br>Error: " +
        e.toString() +
        "</b><br><br>OpenJsCad currently requires Google Chrome or Firefox with WebGL enabled";
    }
    //Zoom control
    if (0) {
      var div = document.createElement("div");
      ...
      this.viewer.onZoomChanged = function () {
        if (!that.zoomChangedBySlider) {
          ...
          that.zoomControl.scrollLeft =
            newzoom * (10 * that.zoomControl.offsetWidth);
        }
      };
      ...
      this.zoomControl.scrollLeft =
        (this.viewer.viewpointZ / this.viewer.ZOOM_MAX) *
    ...
    this.formatDropdown.onchange = function (e) {
      that.currentFormat =
        that.formatDropdown.options[that.formatDropdown.selectedIndex].value;
  ...
  setCurrentObject: function (obj) {
    ...
    if (this.viewer) {
      ...
      this.viewer.state = 2;
      if (obj.length)
        // if it was an array (multiple CSG is now one CSG), we have to reassign currentObject
    ...
    this.supportedFormatsForCurrentObject().forEach(function (format) {
      ...
      option.appendChild(
        document.createTextNode(that.formatInfo(format).displayName)
      );
  ...
  enableItems: function () {
    ...
    this.formatDropdown.style.display =
      !this.hasOutputFile && this.currentObject ? "inline" : "none";
    this.generateOutputFileButton.style.display =
      !this.hasOutputFile && this.currentObject ? "inline" : "none";
    this.downloadOutputFileLink.style.display = this.hasOutputFile
      ? "inline"
      : "none";
    this.parametersdiv.style.display =
      this.paramControls.length > 0 ? "inline-block" : "none"; // was 'block'
  ...
  rebuildSolid: function () {
    ...
    this.statusspan.innerHTML =
      "Rendering code, please wait <img id=busy src='imgs/busy.gif'>";
    this.enableItems();
    var that = this;
    ...
    if (!useSync) {
      try {
        ...
        this.worker = OpenJsCad.parseJsCadScriptASync(
          this.script,
          paramValues,
          this.options,
          function (err, obj) {
    ...
    if (useSync) {
      try {
        ...
        this.statusspan.innerHTML =
          "Rendering code, please wait <img id=busy src='imgs/busy.gif'>";
        var obj = OpenJsCad.parseJsCadScriptSync(
          this.script,
          paramValues,
          this.debugging
        );
  ...
  generateOutputFileBlobUrl: function () {
    ...
    this.downloadOutputFileLink.innerHTML =
      this.downloadLinkTextForCurrentObject();
    var ext = this.selectedFormatInfo().extension;
  ...
  generateOutputFileFileSystem: function () {
    ...
    if (!window.requestFileSystem) {
      throw new Error(
        "Your browser does not support the HTML5 FileSystem API. Please try the Chrome browser instead."
      );
    }
                ...
                fileEntry.createWriter(
                  function (fileWriter) {
                    fileWriter.onwriteend = function (e) {
                      ...
                      that.downloadOutputFileLink.type =
                        that.selectedFormatInfo().mimetype;
                      that.downloadOutputFileLink.innerHTML =
                        that.downloadLinkTextForCurrentObject();
                      that.downloadOutputFileLink.setAttribute(
                        "download",
                        fileEntry.name
                      );
                      that.enableItems();
                  ...
                  function (fileerror) {
                    OpenJsCad.FileSystemApiErrorHandler(
                      fileerror,
                      "createWriter"
                    );
                  }
  ...
  createParamControls: function () {
    ...
    for (var i = 0; i < this.paramDefinitions.length; i++) {
      ...
      if (
        ...
        if (type == "number") control.type = "number";
        else control.type = "text";
        if ("default" in paramdef) {
          control.value = paramdef["default"];
        } else if ("initial" in paramdef) control.value = paramdef.initial;
        else {
        ...
        if (paramdef.size !== undefined) control.size = paramdef.size;
        ...
        if ("captions" in paramdef) {
          ...
          if (captions.length != values.length) {
            throw new Error(
              errorprefix +
                "'captions' and 'values' should have the same number of items"
            );
          }
        } else {
          captions = values;
